...
function fetchJson(url) {
  return new Promise((resolve, reject) => {
    const req = https.get(url, { headers: { 'User-Agent': 'ffxiv-craft-data-updater' }, timeout: 15000 }, (res) => {
      ...
      if (res.statusCode !== 200) return reject(new Error(url + ' 回應 ' + res.statusCode));
      let data = '';
      res.on('data', (c) => { data += c; });
      res.on('end', () => {
        try { resolve(JSON.parse(data)); } catch (e) { reject(new Error(url + ' 不是合法JSON: ' + e.message)); }
      });
    });
...
function extractStatBonus(entry) {
  return { percent: entry.Value, cap: entry.Max, percentHQ: entry.ValueHQ != null ? entry.ValueHQ : entry.Value, capHQ: entry.MaxHQ != null ? entry.MaxHQ : entry.Max };
}
