 ...
 *
 * 修正：
 *  1. F5 閃爍：IIFE 最頂端同步隱藏 body，切換分頁後同步顯示
 *              不依賴 CSS opacity（有繪製時序問題），改用 JS 直接控制
 *  2. 時尚圖片：_parseFashionClean 同時收集圖片 URL（data-src / src）
 *              _renderFashionPosts 交錯渲染文字段落與圖片
 */
(function () {

  console.log('[XIV Patch] v14 loaded');

  /* ══ 立即同步隱藏 body，防止 home 閃爍 ══════════════════════════
   * app_patch.js 在 </body> 前執行，此時 DOM 已建立但尚未完成繪製，
   * 同步設定 visibility:hidden 可確保 home 的錯誤畫面不會被看到。
   * ════════════════════════════════════════════════════════════════ */
  ...
  app.init = function () {
    ...
    /* 同步切換到正確分頁，切完再顯示 body，完全沒有閃爍 */
    var lastNav = sessionStorage.getItem('xiv_nav') || 'home';
    this.nav(lastNav);
    document.body.style.visibility = '';   /* 顯示：不帶 transition，即時 */
  ...
  app.fetchFashion = async function () {
    ...
    if (html1) {
      ...
      ctn.innerHTML = '<div class="news-error"><span>無法連線，<button class="fashion-retry-btn" onclick="app._retryFashion()">重新讀取</button>　或　<a href="' + BASE + '" target="_blank" style="color:var(--gold)">前往原文查看</a></span></div>';
      return;
    }

    /* 只抓最後 3 頁，每頁都明確請求（不重用 html1） */
    var pages = [];
    ...
    if (!final.length) {
      ctn.innerHTML = '<div class="news-error"><span>未找到本週答案，<button class="fashion-retry-btn" onclick="app._retryFashion()">重新讀取</button>　或　<a href="' + BASE + '" target="_blank" style="color:var(--gold)">前往原文查看</a></span></div>';
      return;
    }
