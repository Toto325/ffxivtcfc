 ...
 *
 * 關鍵修正：
 *  1. _fetch：corsproxy.io 改為第一優先（allorigins 持續失敗，先跳過）
 *  2. _parseFashionClean：容器查找改用 closest('section, article')
 *     （原版工作代碼的做法，比 walk-up 可靠）
 *  3. fetchFashion：沿用工作版邏輯，替換解析器為乾淨版，加入快取
 *  4. fetchNews：加入 stale-while-revalidate 快取
 */
(function () {

  /* ══ _fetch：corsproxy.io 優先 ══════════════════════════════════
   * allorigins.win 持續回傳 500/CORS error，排在後面節省時間
   * ══════════════════════════════════════════════════════════════ */
  ...
  /* ══ fetchFashion ═══════════════════════════════════════════════
   * 沿用工作版代碼邏輯（已確認能成功抓取）
   * 替換解析器為 _parseFashionClean（移除 LV/GP/子回覆）
  ...
  app.fetchFashion = async function () {
    ...
    /* 工作版邏輯：抓第 1 頁，用 a[data-floor] 推算最後頁碼 */
    ...
    for (var i = 0; i < pagesToFetch.length; i++) {
      ...
      /* 工作版原始邏輯：lastPage 直接用 html0，避免重複請求 */
      var h = (pg === lastPage && html0) ? html0 : await this._fetch(BASE + '&page=' + pg);
  ...
  /* ══ _parseFashionClean ═════════════════════════════════════════
   * 修正：容器查找改用 closest('section, article')
   *       與工作版原版相同，比 walk-up 可靠
   * 額外移除：.usericon（LV/GP）、.reply-content（子回覆）、
   *           a.count（投票）、.edittime、.buttonbar
   * ══════════════════════════════════════════════════════════════ */
