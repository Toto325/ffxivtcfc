...
app._craftReady = function () {
      ...
      if (summaryEl) {
        ...
        if (!settings.worldName) {
          ...
          html += '<br>整批成品預估收入：<strong>' + Math.round(fin.netRevenue).toLocaleString() + '金</strong>' +
            ...
            '　整批預估淨利：<strong style="color:' + (fin.profit >= 0 ? '#4ade80' : '#f87171') + '">' + (fin.profit >= 0 ? '+' : '') + Math.round(fin.profit).toLocaleString() + '金</strong>' +
  ...
  function fillProfitReference(recipe) {
    ...
    Promise.all([costPromise, sellPromise, taxPromise, velocityPromise]).then(function (out) {
      ...
      if (sellRef) {
        ...
        line1.push('<i class="ph ph-tag"></i>' + settings.worldName + '賣出約 <strong>' + sellRef.price.toLocaleString() + '金</strong>' +
          (taxInfo ? '<span style="opacity:.7">（已扣' + (Math.round(taxInfo.ratePercent * 10) / 10) + '%賣方稅，稅後約' + Math.round(netSell).toLocaleString() + '金）</span>' : '<span style="opacity:.7">（稅率查詢失敗，此數字未扣賣方稅）</span>') +
  ...
  /* 切換求解目標後的視覺回饋：優先在抽屜裡固定不動的「目前求解對象」工具列上閃一下背景色——
   * 那就在使用者眼前，不用往螢幕角落找。只有那個工具列不存在或沒顯示（例如在主界面自己的材料清單
   * 裡切換，不是在抽屜裡）才退回用浮動小提示。 */
  let toastTimer = null;
  function showToast(text) {
    let el = document.getElementById('craft-toast');
    if (!el) {
      el = document.createElement('div');
      el.id = 'craft-toast';
  ...
  let flashTimer = null;
  function flashSwitchConfirm(name) {
    const box = $('cf-explore-current-target');
    if (box && box.offsetParent !== null) { // offsetParent為null代表目前沒顯示（抽屜沒開），這時閃了也沒人看得到
      box.classList.remove('craft-switch-flash');
      requestAnimationFrame(function () { box.classList.add('craft-switch-flash'); });
      if (flashTimer) clearTimeout(flashTimer);
      flashTimer = setTimeout(function () { box.classList.remove('craft-switch-flash'); }, 900);
      return;
    }
    showToast('已切換求解目標：' + name);
  }

  /* 求解目標切換：統一換成榔頭小圖示（原本先改成點名字，但太不明顯，這次改回實體按鈕，
   * 只是圖示化＋不再自動關抽屜/捲動）。材料圖譜、依材料、依世界、自行製作區塊、
   * 主界面自己的材料清單、批次清單最上面的目標成品列表——全部6個出現求解的地方都套用同一套，
   * 不再是「批次相關的地方是新行為，其他地方還是舊行為」這種不一致的體驗。 */
  function bindSwitchRecipeIcons(container) {
    container.querySelectorAll('[data-switch-recipe]').forEach(function (btn) {
      ...
      btn.addEventListener('click', function (e) {
        ...
        const found = RECIPE_INDEX.find(function (r) { return r.id === rid; });
        const ok = selectRecipeById(rid);
        if (ok) flashSwitchConfirm(found ? found.name : '');
      });
    });
  }
