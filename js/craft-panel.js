...
var SPECIAL_ACTIONS = [
   ...
   * 跟一般8個生產職配方完全是不同的遊戲模式，所以求解一般配方、手動操作一般配方時都不該預設可用。
...
app._craftReady = function () {
  ...
  root.innerHTML =
      ...
      '<div class="craft-col craft-col-left">' +
        '<div class="craft-card">' +
          '<div class="craft-row craft-job-row">' +
            '<span class="craft-job-select-label">技能判斷</span>' +
          ...
          '</div>' +
          '<p id="cf-profile-hint-off" class="craft-hint">數值不隨職業切換改變，只用來判斷技能。</p>' +
          '<p id="cf-profile-hint-on" class="craft-hint" style="display:none">切換職業自動套用該職業存的數值與可做清單。</p>' +
    ...
    '<button type="button" id="cf-explore-trigger" class="craft-explore-fab"><i class="ph ph-compass"></i>圖譜與規劃<span id="cf-batch-badge" class="craft-explore-badge" style="display:none">0</span></button>' +
  ...
  function updateStatsScopeTag() {
    ...
    if ($('cf-profile-mode').checked) {
      ...
      tag.innerHTML = '<i class="ph ph-globe"></i>通用數值（不分職業，切換職業不會改變）';
  ...
  function renderSpecialActions() {
    ...
    box.querySelectorAll('[data-special]').forEach(function (cb) {
      cb.addEventListener('change', function () {
        ...
        /* 原本勾/取消勾選之後，手動操作那邊的技能格子跟搜尋結果的可行性判斷都沒有跟著重畫，
  ...
  ['cf-craftsmanship', 'cf-control', 'cf-cp', 'cf-food', 'cf-pot'].forEach(function (id) {
    ...
    /* 這幾個數值都會直接影響手動操作「哪些技能現在按得下去」，原本改了數值手動操作那邊完全沒反應，
  ...
  $('cf-solve-prev').addEventListener('click', function () { stepSolveHistory(-1); });
  $('cf-solve-next').addEventListener('click', function () { stepSolveHistory(1); });
  ...
  /* 上一個/下一個導覽列：兩顆按鈕文字直接顯示上一個/下一個是什麼物品，沒有可切換的方向就整顆隱藏，
   * 不要留一個按下去沒反應的死按鈕。 */
  function renderSolveNav() {
    const nav = $('cf-solve-nav');
    if (!selectedRecipe || solveHistory.length <= 1) { nav.style.display = 'none'; return; }
    const hasPrev = solveHistoryIndex > 0;
    const hasNext = solveHistoryIndex < solveHistory.length - 1;
    if (!hasPrev && !hasNext) { nav.style.display = 'none'; return; }
    nav.style.display = 'flex';
    const prevBtn = $('cf-solve-prev'), nextBtn = $('cf-solve-next');
  ...
  function renderBatchSeqNav() {
    ...
    const hasNext = batchSeqIndex >= 0 && batchSeqIndex < batchList.length - 1;
    const posLabel = batchSeqIndex >= 0 ? ('第 ' + (batchSeqIndex + 1) + ' / ' + batchList.length + ' 項') : ('依序求解（共 ' + batchList.length + ' 項）');
    nav.innerHTML =
  ...
  function stepBatchSeq(dir) {
    ...
    selectRecipeById(batchList[batchSeqIndex].recipeId);
  ...
  $('cf-m-reset').addEventListener('click', function () {
    ...
    if (!confirm('要重置手動操作的全部步驟嗎？目前已經排的 ' + manualRotation.length + ' 步會全部清空，這個動作沒辦法復原。')) return;
    manualRotation = [];
    renderManual();
  });
  ...
  function diagnoseFailure(craft, result, stats, hqIngredients) {
    ...
    if (progressGap > 0) {
      if (result.progression > 0 && result.actions && result.actions.length) {
        const need = estimateStatNeeded(craft, result.actions, stats, hqIngredients, 'craftsmanship', function (simResult) { return simResult.progression >= craft.progress; });
        tips.push(need != null
        ...
        const cpNeed = estimateStatNeeded(craft, result.actions, stats, hqIngredients, 'cp', function (simResult) { return simResult.progression >= craft.progress; });
      ...
      if (result.quality > 0 && result.actions && result.actions.length) {
        const need = estimateStatNeeded(craft, result.actions, stats, hqIngredients, 'control', function (simResult) { return simResult.quality >= craft.quality; });
        tips.push(need != null
        ...
        const cpNeed = estimateStatNeeded(craft, result.actions, stats, hqIngredients, 'cp', function (simResult) { return simResult.quality >= craft.quality; });
        if (cpNeed != null && cpNeed > stats.cp) {
          tips.push('或者：CP加到約 <strong>' + cpNeed + '</strong>（目前 ' + stats.cp + '）配合同一套手法也可能補滿（同樣是實際重新模擬算出來的）');
          tips.push(cpNote);
        }
  ...
  function renderResult(craft, result, solvingName, stats, hqIngredients) {
    ...
    const tips = (stats && (!result.success || hasQualityGap)) ? diagnoseFailure(craft, result, stats, hqIngredients) : [];
    box.innerHTML =
