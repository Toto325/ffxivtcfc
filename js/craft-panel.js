...
app._craftReady = function () {
  ...
  async function ensureMarketSettingsUi() {
    ...
    try {
      ...
      $('cf-market-world').addEventListener('change', function () {
        MarketData.setWorld($('cf-market-world').value);
      });
  ...
  function fillNodePrices(container) {
    ...
    nodes.forEach(function (el) {
      ...
      el.textContent = '查價中…';
      MarketData.getUnitPriceRef(itemId).then(function (ref) {
        if (!ref) { el.textContent = ''; return; }
        el.textContent = ref.price.toLocaleString() + '金／' + ref.world;
  ...
  function fillBatchCosts(rows) {
    ...
    MarketData.resolveBuyCosts(needMap).then(function (results) {
      let total = 0;
      let shortageCount = 0;
      Object.keys(results).forEach(function (itemId) {
        ...
        const detailEl = document.querySelector('[data-worlds-item="' + itemId + '"]');
        if (cell) {
          if (!r.filledQty) {
            ...
            // 跨多個世界湊齊：主要顯示總金額，明細（各世界買幾個、單價多少）收在可展開的區塊，避免清單一次塞太多字
        ...
        if (detailEl && r.breakdown && r.breakdown.length) {
          detailEl.innerHTML = r.breakdown.map(function (b) {
            return '<span class="craft-mat-world-item">' + b.world + '　×' + b.qty.toLocaleString() + '（' + b.unitPrice.toLocaleString() + '金/個）</span>';
          }).join('');
        }
        if (r.totalCost) total += r.totalCost;
        if (!r.resolved) shortageCount++;
      });
      // 世界明細預設收合，點「跨N世界▾」才展開，用事件委派掛在整個清單容器上，避免每次重繪都要重新綁定
      const container = $('cf-batch-materials');
      if (container && !container.dataset.worldsToggleBound) {
        container.dataset.worldsToggleBound = '1';
        container.addEventListener('click', function (ev) {
          const btn = ev.target.closest('[data-worlds-toggle]');
          if (!btn) return;
          const detail = document.querySelector('[data-worlds-item="' + btn.dataset.worldsToggle + '"]');
          if (!detail) return;
      ...
      if (summaryEl) {
        summaryEl.innerHTML = '整批預估採購成本：<strong>' + Math.round(total).toLocaleString() + '金</strong>' +
    ...
    $('cf-batch-materials').innerHTML =
      ...
      (rows.map(function (r) {
        ...
        return '<div class="craft-mat-buy-block" data-buy-item="' + r.itemId + '">' +
          ...
          (rid ? '<button type="button" class="craft-mat-switch" data-switch-recipe="' + rid + '" title="切換成求解對象">求解</button>' : '') +
          toggle + '</span></div>' +
          '<div class="craft-mat-worlds" data-worlds-item="' + r.itemId + '" style="display:none"></div>' +
          '</div>';
