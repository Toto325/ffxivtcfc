...
app._craftReady = function () {
  ...
  function fillNodePrices(container) {
    ...
    const nodes = container.querySelectorAll('[data-price-item]:not([data-price-filled])');
    nodes.forEach(function (el) {
      el.setAttribute('data-price-filled', '1');
      const itemId = el.dataset.priceItem;
      const recipeId = el.dataset.craftRecipe;
      el.textContent = '查價中…';
      const buyPromise = MarketData.getUnitPriceRef(itemId);
      const craftPromise = recipeId && CRAFT_RECIPES[recipeId]
        ? (function () {
            const needMap = {};
            (CRAFT_RECIPES[recipeId].ingredients || []).forEach(function (ing) { needMap[ing.itemId] = ing.amount; });
            return Object.keys(needMap).length ? MarketData.resolveBuyCosts(needMap) : Promise.resolve(null);
          })()
        : Promise.resolve(null);

      Promise.all([buyPromise, craftPromise]).then(function (out) {
        const ref = out[0], craftResults = out[1];
        if (!ref) { el.textContent = ''; return; }
        let html = ref.price.toLocaleString() + '金／' + ref.world;
        el.title = '該世界目前最低掛單價，僅供參考';
        if (craftResults) {
          let craftCost = 0, allResolved = true;
          Object.keys(craftResults).forEach(function (id) {
            const r = craftResults[id];
            craftCost += r.totalCost || 0;
            if (!r.filledQty) allResolved = false;
          });
          if (allResolved && craftCost > 0) {
            if (craftCost < ref.price) {
              html += ' <span class="craft-mat-decision craft-mat-decision-craft">自己做省' + Math.round(ref.price - craftCost).toLocaleString() + '金</span>';
            } else {
  ...
  function fillBatchCosts(rows) {
    ...
    computeBatchFinancials(rows).then(function (fin) {
      ...
      // 跟直接買（已經算好在results裡）比較，放在summary計算之外另外跑，不要卡住主要成本/淨利的顯示。
      const craftableRows = rows.filter(function (r) { return r.craftable && (ITEM_TO_RECIPES[r.itemId] || [])[0]; });
      Promise.all(craftableRows.map(function (r) {
        const rid = (ITEM_TO_RECIPES[r.itemId] || [])[0];
        const recipe = CRAFT_RECIPES[rid];
        const subNeed = {};
        (recipe.ingredients || []).forEach(function (ing) { subNeed[ing.itemId] = ing.amount * r.amount; });
        if (!Object.keys(subNeed).length) return Promise.resolve(null);
        return MarketData.resolveBuyCosts(subNeed).then(function (res) {
          let craftCost = 0, allResolved = true;
          Object.keys(res).forEach(function (id) {
            const rr = res[id];
            craftCost += rr.totalCost || 0;
            if (!rr.filledQty) allResolved = false;
          });
          const buyEntry = results[r.itemId];
          return { itemId: r.itemId, craftCost: craftCost, allResolved: allResolved, buyCost: buyEntry ? buyEntry.totalCost : null };
        });
      })).then(function (decisions) {
        decisions.forEach(function (d) {
          if (!d || !d.allResolved || d.buyCost == null) return;
          const hintEl = document.querySelector('[data-decision-item="' + d.itemId + '"]');
          if (!hintEl) return;
          if (d.craftCost < d.buyCost) {
            hintEl.innerHTML = '<span class="craft-mat-decision craft-mat-decision-craft">省' + Math.round(d.buyCost - d.craftCost).toLocaleString() + '金</span>';
          } else {
  ...
  async function autoSuggestBatch() {
    ...
    try {
      for (let iter = 0; iter < 6; iter++) {
        ...
        const craftDecisionsPromise = Promise.all(candidates.map(function (r) {
          const rid = (ITEM_TO_RECIPES[r.itemId] || [])[0];
          const recipe = CRAFT_RECIPES[rid];
          const subNeed = {};
          (recipe.ingredients || []).forEach(function (ing) { subNeed[ing.itemId] = ing.amount * r.amount; });
        ...
        const out = await Promise.all([buyResultsPromise, craftDecisionsPromise]);
        const buyResults = out[0], decisions = out[1];
