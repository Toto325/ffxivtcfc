...
app._craftReady = function () {
      ...
      nodes.forEach(function (el) {
        ...
        if (ings) {
          ...
          if (allResolved && craftCost > 0) {
            if (craftCost < r.totalCost) {
              html += ' ' + decisionDotHtml('craft', '自己做省' + Math.round(r.totalCost - craftCost).toLocaleString() + '金');
            } else {
              html += ' ' + decisionDotHtml('buy', '買比較划算，省' + Math.round(craftCost - r.totalCost).toLocaleString() + '金');
            }
    ...
    computeBatchFinancials(rows).then(function (fin) {
      ...
      Object.keys(results).forEach(function (itemId) {
        ...
        if (cell) {
          ...
          } else if (r.worldCount <= 1) {
            // 單一世界就能湊齊，直接把世界名稱標在價格旁邊，不用額外點開
            const w = r.breakdown[0] ? r.breakdown[0].world : '';
            cell.textContent = Math.round(r.totalCost).toLocaleString() + '金／' + w + (r.resolved ? '' : '⚠部分缺貨');
          } else {
            // 跨多個世界湊齊：主要顯示總金額，明細另外點按鈕彈出小卡看，避免清單一次塞太多字
            cell.innerHTML = Math.round(r.totalCost).toLocaleString() + '金 ' +
              '<button type="button" class="craft-mat-worlds-toggle" data-worlds-toggle="' + itemId + '">跨' + r.worldCount + '世界▾</button>' +
              (r.resolved ? '' : '⚠部分缺貨');
          }
        }
      });
      ...
      if (Object.keys(combinedSubNeed).length) {
        MarketData.resolveBuyCosts(combinedSubNeed).then(function (combinedResults) {
          craftableRows.forEach(function (r) {
            ...
            const dotHtml = craftCost < buyEntry.totalCost
              ? decisionDotHtml('craft', '自己做省' + Math.round(buyEntry.totalCost - craftCost).toLocaleString() + '金')
              : decisionDotHtml('buy', '買划算，省' + Math.round(craftCost - buyEntry.totalCost).toLocaleString() + '金');
            hintEls.forEach(function (hintEl) {
              hintEl.innerHTML = dotHtml;
              bindDecisionDots(hintEl);
            });
          });
  ...
  function renderByWorldView(results, nameByItem, rows, craftRows) {
    ...
    box.innerHTML = craftSection + worldNames.map(function (w) {
      ...
      return '<div class="craft-byworld-card">' +
        ...
        items.map(function (e) {
          ...
          // 跟「依材料」用同一組按鈕（求解／自己做／決策小圓點），視覺與操作盡量對齊，只是排序焦點換成世界
          const toggleBtn = info.craftable
            ? '<button type="button" class="craft-copy-btn" data-expand-toggle="' + e.itemId + '">自己做</button>'
            : '';
          return '<div class="craft-byworld-item"><span class="craft-ing-name">' + e.name + '</span>' +
            '<span class="craft-row" style="gap:6px"><span class="craft-muted">×' + e.qty.toLocaleString() + '</span>' +
            '<span class="craft-mat-price">' + e.unitPrice.toLocaleString() + '金/個</span>' +
            (info.rid ? '<button type="button" class="craft-mat-switch" data-switch-recipe="' + info.rid + '" title="切換成求解對象">求解</button>' : '') +
            toggleBtn +
            (info.craftable ? '<span class="craft-mat-decision-hint" data-decision-item="' + e.itemId + '"></span>' : '') +
    ...
    bindMatActions(box); // 求解按鈕＋決策小圓點跟「依材料」共用同一套綁定邏輯
    bindExpandCollapseToggles(box); // 自己做按鈕（改採購那顆在自行製作區塊裡，已經含在craftSection內）
  }
  ...
  function fillProfitReference(recipe) {
    ...
    Promise.all([costPromise, sellPromise, taxPromise, velocityPromise]).then(function (out) {
      ...
      const parts = [];
      parts.push('<i class="ph ph-package"></i>材料成本約 <strong>' + Math.round(materialCost).toLocaleString() + '金</strong>' +
        '<span style="opacity:.7">（已含買方稅）</span>' +
      ...
      if (sellRef) {
        ...
        // 抓不到稅率資料時退回不扣稅，並在文字上註明，避免顯示一個沒說清楚假設的數字。
        const rate = taxInfo ? taxInfo.ratePercent : 0;
        const netSell = sellRef.price * (1 - rate / 100);
        const profit = netSell - materialCost;
        parts.push('<i class="ph ph-tag"></i>' + settings.worldName + '賣出約 <strong>' + sellRef.price.toLocaleString() + '金</strong>' +
          (taxInfo ? '<span style="opacity:.7">（' + taxInfo.cityName + '寄售' + (taxInfo.isUserChosen ? '' : '・自動選最低稅率') + '，稅後約' + Math.round(netSell).toLocaleString() + '金）</span>' : '<span style="opacity:.7">（稅率查詢失敗，此數字未扣賣方稅）</span>') +
        ...
        // 淨利＋銷售速度合併成同一行，這兩個數字放一起看最直接（賺多少、賣得掉賣不掉）
        parts.push('預估淨利 <strong style="color:' + (profit >= 0 ? '#4ade80' : '#f87171') + '">' + (profit >= 0 ? '+' : '') + Math.round(profit).toLocaleString() + '金</strong>' +
          '<span style="opacity:.7">／件（僅單件製作，不含製作時間成本）</span>' + velocityText);
      } else if (!settings.worldName) {
        parts.push('<span style="opacity:.7">未設定「我的世界」，看不到賣出參考與淨利</span>' + velocityText);
      } else {
        // 世界其實已經設定好了，只是這個物品目前該世界查不到掛單（可能沒人在賣，或剛好賣光），
        // 不能跟「根本沒設定世界」用同一句話講，不然玩家會誤以為是自己忘了設定。
        parts.push('<span style="opacity:.7">' + settings.worldName + '目前查無掛單（可能沒有人在賣），看不到賣出參考與淨利</span>' + velocityText);
      }
  ...
  /* 買/做決策小圓點：兩處（材料圖譜節點、批次列表）共用同一顆產生函式，樣式/行為才不會兩邊各做各的。
   * 平常只是一顆小圓點，滑鼠移過去或點一下才彈出文字，不像之前的文字徽章那樣直接佔用版面。 */
  function decisionDotHtml(kind, text) {
    return '<button type="button" class="craft-decision-dot craft-decision-dot-' + kind + '" data-decision-toggle title="' + text + '"><span class="craft-decision-tip">' + text + '</span></button>';
  }
  function bindDecisionDots(container) {
    container.querySelectorAll('[data-decision-toggle]').forEach(function (btn) {
      if (btn._dotBound) return; // 重複render時避免重複綁定同一顆按鈕
      btn._dotBound = true;
      btn.addEventListener('click', function (e) {
        e.stopPropagation();
        const wasOpen = btn.classList.contains('show');
        document.querySelectorAll('.craft-decision-dot.show').forEach(function (o) { o.classList.remove('show'); });
        if (!wasOpen) btn.classList.add('show');
      });
    });
  }

  function bindMatActions(container) {
    bindDecisionDots(container);
  ...
  function renderBatch() {
    ...
    $('cf-batch-materials').innerHTML =
      ...
      (rows.map(function (r) {
        ...
        // 跟材料圖譜節點同樣的道理：按鈕收一排、價格收下面一排，兩排都收在靠右的直立小欄位裡，
        // 這樣價格＋決策標籤要多長都只影響這個小欄位自己，不會拖著左邊的名稱/數量欄位一起跑版，
        // 同時維持「價格在右邊」比較好讀的排版習慣。
        return '<div class="craft-mat-buy-block" data-buy-item="' + r.itemId + '">' +
          '<div class="craft-ing-row" data-buy-qty="' + r.amount + '"><span class="craft-row" style="gap:8px"><span class="craft-ing-name">' + r.name + '</span><span class="craft-muted">×' + r.amount.toLocaleString() + '</span></span>' +
          '<div class="craft-mat-right-col">' +
            '<div class="craft-row" style="gap:8px">' +
              (rid ? '<button type="button" class="craft-mat-switch" data-switch-recipe="' + rid + '" title="切換成求解對象">求解</button>' : '') +
              toggleBtn +
            '</div>' +
            '<span class="craft-mat-price" data-buy-cost-item="' + r.itemId + '"></span>' +
            (r.craftable ? '<span class="craft-mat-decision-hint" data-decision-item="' + r.itemId + '"></span>' : '') +
          '</div>' +
          '</div>' +
          '</div>';
