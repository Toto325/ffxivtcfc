...
app._craftReady = function () {
        ...
        '<div class="craft-explore-body">' +
          ...
          '<div id="cf-explore-batch" class="craft-explore-pane" style="display:none">' +
            ...
            '<div class="craft-ingredients"><p class="craft-ingredients-title">整批訂單規劃　<span class="craft-muted">材料彙總／購物清單，可選「自己做」往下展開，或維持採購</span></p>' +
              '<div class="craft-row" style="gap:8px;margin:2px 0 6px"><button type="button" id="cf-batch-autosuggest" class="craft-batch-autosuggest-btn"><i class="ph ph-magic-wand"></i>自動建議</button><span id="cf-batch-autosuggest-status" class="craft-muted" style="font-size:11px"></span></div>' +
              ...
              '<div class="craft-row" style="gap:6px;margin-bottom:6px">' +
  ...
  function openWorldPopover(btn, itemId) {
    ...
    const breakdown = worldBreakdownCache[itemId] || [];
    ...
    breakdown.forEach(function (b) {
      if (!grouped[b.world]) { grouped[b.world] = { qty: 0, entries: [] }; worldOrder.push(b.world); }
      grouped[b.world].qty += b.qty;
      grouped[b.world].entries.push(b);
    });
    pop.innerHTML = '<p class="craft-mat-worlds-title">分' + worldOrder.length + '個世界湊齊，由便宜到貴：</p>' +
      worldOrder.map(function (w) {
        const g = grouped[w];
        if (g.entries.length === 1) {
          return '<div class="craft-mat-world-row"><span class="w-name">' + w + '</span><span class="w-qty">×' + g.qty.toLocaleString() + '</span><span class="w-price">' + g.entries[0].unitPrice.toLocaleString() + '金/個</span></div>';
        }
        return '<div class="craft-mat-world-group">' +
          '<div class="craft-mat-world-row"><span class="w-name">' + w + '</span><span class="w-qty">共×' + g.qty.toLocaleString() + '</span></div>' +
          g.entries.map(function (e) {
            return '<div class="craft-mat-world-subrow"><span class="w-qty">×' + e.qty.toLocaleString() + '</span><span class="w-price">' + e.unitPrice.toLocaleString() + '金/個</span></div>';
          }).join('') +
          '</div>';
      }).join('');
    pop.style.display = 'flex';
  ...
  function fillBatchCosts(rows) {
    ...
    Promise.all([MarketData.resolveBuyCosts(needMap), sellPromise, taxPromise]).then(function (out) {
      ...
      Object.keys(results).forEach(function (itemId) {
        ...
        worldBreakdownCache[itemId] = r.breakdown || [];
        if (cell) {
          if (!r.filledQty) {
      ...
      // 賣出端：批次清單裡每個成品的數量 × 該世界最低掛價，加總成整批預估「稅前」收入；查不到價的項目排除在外並提示，
      // 不要用0代替，避免把淨利算得比實際好看。賣方稅另外套用在總額上（假設整批都在同一個城市寄售，比逐項各自套稅更貼近實際操作習慣）。
      ...
      if (summaryEl) {
        let html = '整批預估採購成本：<strong>' + Math.round(total).toLocaleString() + '金</strong>' +
          '<span style="opacity:.7">（已含買方稅）</span>' +
          (shortageCount > 0 ? '　<span style="color:#d97706">（' + shortageCount + '項市場庫存不足以湊滿數量，成本僅計算查得到的部分）</span>' : '');
        if (!settings.worldName) {
          ...
          const profit = netRevenue - total;
          html += '<br>整批成品預估收入：<strong>' + Math.round(netRevenue).toLocaleString() + '金</strong>' +
            ...
            (missingSell > 0 ? '<span style="opacity:.7">，' + missingSell + '項查無賣出掛單未計入</span>' : '') +
            '　整批預估淨利：<strong style="color:' + (profit >= 0 ? '#4ade80' : '#f87171') + '">' + (profit >= 0 ? '+' : '') + Math.round(profit).toLocaleString() + '金</strong>' +
      ...
      return '<div class="craft-byworld-card">' +
        ...
        items.map(function (e) {
          return '<div class="craft-byworld-item"><span class="craft-ing-name">' + e.name + '</span>' +
            ...
            '<button type="button" class="craft-mat-worlds-toggle" data-worlds-toggle="' + e.itemId + '" title="看看這個材料在其他世界賣多少、還有多少庫存">其他世界</button>' +
            '</span></div>';
        }).join('') +
      '</div>';
    }).join('');
  ...
  function renderBatch() {
     ...
     * 抽成獨立函式是因為「自動建議」也要反覆用同一套expand邏輯試算不同expandedItems組合下的結果，
     * 不想跟畫面渲染耦合在一起。 */
  ...
  async function autoSuggestBatch() {
    ...
    const freshStatusEl = $('cf-batch-autosuggest-status'); // renderBatch重繪過DOM，原本抓到的statusEl可能已經是舊節點
    if (freshStatusEl) {
      freshStatusEl.textContent = totalSwitched > 0
        ? '已自動切換 ' + totalSwitched + ' 項為自己做'
