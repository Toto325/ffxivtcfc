...
app._craftReady = function () {
  ...
  function fillNodePrices(container) {
    ...
    Promise.all([
      ...
      nodes.forEach(function (el) {
        ...
        if (!r || !r.filledQty) { el.textContent = ''; return; }
      ...
      if (summaryEl) {
        ...
        if (!settings.worldName) {
          ...
          html += '<br>整批成品預估收入：<strong>' + Math.round(fin.netRevenue).toLocaleString() + '金</strong>' +
            (taxInfo ? '<span style="opacity:.7">（假設在' + taxInfo.cityName + '寄售' + (taxInfo.isUserChosen ? '' : '・自動選最低稅率') + '，已扣' + taxInfo.ratePercent + '%賣方稅）</span>' : '<span style="opacity:.7">（稅率查詢失敗，此數字未扣賣方稅）</span>') +
            (fin.missingSell > 0 ? '<span style="opacity:.7">，' + fin.missingSell + '項查無賣出掛單未計入</span>' : '') +
    ...
    box.innerHTML = craftSection + worldNames.map(function (w) {
      ...
      return '<div class="craft-byworld-card">' +
        ...
        items.map(function (e) {
          ...
          return '<div class="craft-byworld-item"><span class="craft-ing-name"' + (info.rid ? ' data-switch-recipe-name="' + info.rid + '"' : '') + '>' + e.name + '</span>' +
  ...
  function materialNodeHtml(itemId, direction, seenPath, knownRecipeId) {
    ...
    return '<div class="craft-mat-node" data-item="' + itemId + '" data-dir="' + direction + '" data-path="' + seenPath + '">' +
      '<div class="craft-mat-row">' +
        ...
        '<span class="craft-mat-name"' + (craftableRecipeId ? ' data-switch-recipe-name="' + craftableRecipeId + '"' : '') + '>' + itemIconHtml(itemId, 18) + itemName(itemId) + '</span>' +
  ...
  function decisionCellHtml(mode, craftCost, buyCost) {
    ...
    const otherLabel = mode === 'buy' ? '自己做' : '買';
    if (Math.abs(diff) < 1) return '<span class="craft-decision-num craft-muted">與' + otherLabel + '同價</span>';
    const color = diff > 0 ? '#4ade80' : '#f87171';
    const verb = diff > 0 ? '省' : '貴';
    return '<span class="craft-decision-num" style="color:' + color + '">比' + otherLabel + verb + Math.round(Math.abs(diff)).toLocaleString() + '</span>';
  }

  /* 短暫提示（左上角），給「點名字背景切換求解目標」這種不捲動、不關抽屜的操作一個視覺回饋，
   * 不然使用者會不確定自己剛剛的點擊有沒有生效。 */
  let toastTimer = null;
  function showToast(text) {
    ...
    el.textContent = text;
    // 用rAF確保class變化真的觸發transition，不然class加了又馬上顯示會被瀏覽器合併成沒動畫
  ...
  /* 點物品名稱＝切換求解目標，但保持停留在目前畫面，不關抽屜、不捲動——跟原本「求解」按鈕
   * 會強制關抽屜+捲回頂端的行為不同，這是這次刻意要改成「背景安靜更新」的地方。
   * 材料圖譜、依材料、依世界、自行製作區塊都共用這個綁定，操作體驗才會一致。 */
  function bindNameSwitchTriggers(container) {
    container.querySelectorAll('[data-switch-recipe-name]').forEach(function (el) {
      if (el._switchBound) return;
      el._switchBound = true;
      el.addEventListener('click', function () {
        const rid = el.dataset.switchRecipeName;
        const ok = selectRecipeById(rid);
        if (ok) showToast('已切換求解目標：' + el.textContent.trim());
      });
    });
  }
  ...
  function bindMatActions(container) {
    bindNameSwitchTriggers(container);
  ...
  function craftOnlySectionHtml(craftRows) {
    ...
    return '<p class="craft-mat-section-label">▾ 自行製作（材料已列在下方採購清單）</p>' +
      craftRows.map(function (r) {
        const rid = (ITEM_TO_RECIPES[r.itemId] || [])[0];
        return '<div class="craft-ing-row"><span class="craft-ing-name"' + (rid ? ' data-switch-recipe-name="' + rid + '"' : '') + '>' + r.name + '</span>' +
      ...
      (rows.map(function (r) {
        ...
        return '<div class="craft-mat-buy-block" data-buy-item="' + r.itemId + '">' +
          '<div class="craft-ing-row" data-buy-qty="' + r.amount + '">' +
            '<span class="craft-ing-name"' + (rid ? ' data-switch-recipe-name="' + rid + '"' : '') + '>' + r.name + '</span>' +
            '<span class="craft-row-grid">' +
