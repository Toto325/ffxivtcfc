  ...
  function fillBatchCosts(rows) {
    ...
    MarketData.resolveBuyCosts(needMap).then(function (results) {
      ...
      Object.keys(results).forEach(function (itemId) {
        ...
        if (cell) {
          if (!r.filledQty) { cell.textContent = '查無掛單'; }
          else {
            cell.textContent = Math.round(r.totalCost).toLocaleString() + '金' + (r.worldCount > 1 ? '（跨' + r.worldCount + '世界）' : '');
            if (!r.resolved) cell.textContent += '⚠部分缺貨';
          }
        }
  ...
  function renderBatch() {
    ...
    $('cf-batch-materials').innerHTML =
      ...
      (rows.map(function (r) {
        ...
        return '<div class="craft-ing-row" data-buy-item="' + r.itemId + '" data-buy-qty="' + r.amount + '"><span class="craft-ing-name">' + r.name + '</span><span class="craft-row" style="gap:8px"><span class="craft-muted">×' + r.amount.toLocaleString() + '</span>' +
