...
app._craftReady = function () {
    ...
    '<div class="craft-layout">' +
      '<div class="craft-col craft-col-left">' +
        '<div class="craft-card">' +
          ...
          '</div>' +
          '<label class="craft-check-inline craft-profile-toggle"><input id="cf-profile-mode" type="checkbox" />使用個人設定檔<span class="craft-muted">（幫每個職業各自記住等級／數值，切職業自動套用）</span></label>' +
        '</div>' +
      ...
      '<div class="craft-col craft-col-right">' +
        ...
        '</div>' +
        /* 求解區塊緊接在「選配方＋設定材料HQ」後面，不讓下面可能很長的材料圖譜／批次清單擋住，
         * 求解按鈕不用滑很久才看到。材料圖譜、批次清單是探索/規劃用的附加內容，挪到求解區塊之後。 */
        ...
        '<div id="cf-manual-panel" class="craft-card" style="display:none">' +
          '<div class="craft-row" style="gap:20px">' +
            ...
            '<div style="flex:1"><div class="craft-bar-label"><span>CP</span><span id="cf-m-cp-text">- / -</span></div><div class="craft-bar-track"><div id="cf-m-cp-bar" class="craft-bar-fill craft-fill-quality" style="width:100%"></div></div></div>' +
          '</div>' +
  ...
  ['cf-lvl', 'cf-craftsmanship', 'cf-control', 'cf-cp', 'cf-specialist'].forEach(function (id) {
    $(id).addEventListener('change', function () {
      if ($('cf-profile-mode').checked) saveCurrentToProfile(parseInt($('cf-job').value, 10));
    });
  });
  ...
  /* 快速可行性判斷：只用最單純的「全部技能都拿去衝進度」排法跑一次模擬，不是真的求解找最佳手法，
   * 這樣才夠快，可以用在搜尋清單每一列上。這是保守估計：
  ...
  function quickFeasibility(recipe, crafterStats) {
    try {
      ...
      const steps = Math.max(1, Math.floor(recipe.durability / 10));
      const actions = Array.from({ length: steps }).map(function () { return new simulator.BasicSynthesis(); });
  ...
  function runSearch() {
    ...
    box.innerHTML = matches.map(function (r, i) {
      ...
      return '<div class="craft-recipe-item" data-id="' + r.id + '">' + itemIconHtml(r.itemId, 22) +
        ...
        badge +
        '<span class="craft-muted">Lv' + r.lvl + '　rlvl' + r.rlvl + '</span></div>';
  ...
  /* 材料圖譜節點上的快速加入批次按鈕綁定，跟展開鈕bindToggle一樣，每次渲染新節點都要重綁一次 */
  function bindMatQuickAdd(container) {
  ...
  function renderManual() {
    ...
    $('cf-m-sequence').innerHTML = manualRotation.map(function (a) {
      ...
      return '<span class="craft-m-seq-chip">' + actionIconHtml(a.constructor.name, 16) + (n ? n.tw : a.constructor.name) + '</span>';
    ...
    $('cf-m-skills').innerHTML = actions.map(function (a) {
      ...
      return '<button type="button" class="craft-m-skill-btn" data-key="' + a.constructor.name + '" ' + (usable ? '' : 'disabled') + '>' +
        actionIconHtml(a.constructor.name, 26) + '<span class="craft-m-skill-label">' + label + '</span></button>';
    }).join('');
  ...
  function diagnoseFailure(craft, result, stats) {
    ...
    if (progressGap > 0) {
      ...
      if (qualityGap > 0 && result.quality > 0) {
  ...
  function renderResult(craft, result, solvingName, stats) {
    ...
    const tips = (!result.success && stats) ? diagnoseFailure(craft, result, stats) : [];
    box.innerHTML =
      ...
      '<div class="craft-result-meta">' +
        ...
        '<div class="craft-meta-cp"><div class="craft-bar-label"><span><i class="ph ph-sparkle"></i>CP</span><span>' + result.cpLeft + ' / ' + (lastCpMax || result.cpLeft) + '</span></div><div class="craft-bar-track"><div class="craft-bar-fill craft-fill-quality" style="width:' + Math.max(0, Math.round(result.cpLeft / (lastCpMax || result.cpLeft) * 100)) + '%"></div></div></div>' +
      '</div>' +
      ...
      (tips.length ? '<div class="craft-diagnosis"><p class="craft-diagnosis-title"><i class="ph ph-lightbulb"></i>怎麼樣可能就過了（估計，不是保證）</p><ul>' + tips.map(function (t) { return '<li>' + t + '</li>'; }).join('') + '</ul></div>' : '') +
      (result.success ? (
