...
app._craftReady = function () {
  ...
  root.innerHTML =
    ...
    '<div id="cf-explore-drawer" class="craft-explore-drawer">' +
      ...
      '<div class="craft-explore-panel">' +
        ...
        /* 市場伺服器設定：買材料查整個資料中心、賣成品查玩家自己選的世界，兩支select各自負責。
         * 放在抽屜最上面，不管切到哪個分頁籤都看得到、隨時能改，不用另外跳頁設定。 */
        '<div id="cf-market-settings" class="craft-row" style="gap:8px;flex-wrap:wrap;padding:0 2px 8px">' +
          '<span class="craft-label"><i class="ph ph-currency-circle-dollar"></i>市場</span>' +
          '<select id="cf-market-dc" class="craft-select"><option value="">資料中心⋯</option></select>' +
          '<select id="cf-market-world" class="craft-select"><option value="">我的世界（賣出用）⋯</option></select>' +
          '<select id="cf-market-city" class="craft-select" style="display:none"></select>' +
          '<span class="craft-muted" style="font-size:11px">材料成本查整個資料中心最低價；賣出參考只看你自己的世界</span>' +
        '</div>' +
        '<div class="craft-explore-body">' +
          ...
          '<div id="cf-explore-batch" class="craft-explore-pane" style="display:none">' +
            ...
            '<div class="craft-ingredients"><p class="craft-ingredients-title">整批訂單規劃　<span class="craft-muted">材料彙總／購物清單，可選「自己做」往下展開，或維持採購</span></p>' +
              '<div class="craft-row" style="gap:8px;margin:2px 0 6px"><button type="button" id="cf-batch-autosuggest" class="craft-copy-btn"><i class="ph ph-magic-wand"></i>自動建議</button><span id="cf-batch-autosuggest-status" class="craft-muted" style="font-size:11px"></span></div>' +
              '<div id="cf-batch-materials"></div></div>' +
            '<div id="cf-world-popover" class="craft-mat-worlds-popover" style="display:none"></div>' +
          '</div>' +
        '</div>' +
      '</div>' +
    '</div>';
  ...
  /* ── 市場伺服器設定：DC決定「買」的查詢範圍，世界決定「賣」的查詢範圍 ──
   * 兩個select的內容都是跟Universalis即時要來的，不在這裡手刻寫死伺服器名單，
   * 避免拼字/未來改版跟實際資料兜不起來。第一次開抽屜才去要清單，不影響頁面首次載入速度。 */
  let marketMetaLoaded = false;
  let worldBreakdownCache = {}; // itemId -> breakdown[]，給世界明細彈出卡讀取用，不用每次點按鈕都重查
  let worldPopoverBound = false;
  async function ensureMarketSettingsUi() {
    if (marketMetaLoaded || typeof MarketData === 'undefined') return;
    marketMetaLoaded = true;
    try {
      const dcNames = await MarketData.listDcNames();
      const dcSel = $('cf-market-dc');
      dcSel.innerHTML = '<option value="">資料中心⋯</option>' +
        dcNames.map(function (n) { return '<option value="' + n + '">' + n + '</option>'; }).join('');
      const saved = MarketData.getSettings();
      if (saved.dcName) { dcSel.value = saved.dcName; await refreshWorldOptions(saved.dcName, saved.worldName); }
      if (saved.worldName) await refreshCityOptions(saved.worldName, saved.sellCityKey);
      dcSel.addEventListener('change', async function () {
        await MarketData.setDataCenter(dcSel.value);
        await refreshWorldOptions(dcSel.value, null);
        await refreshCityOptions(null, null); // 換DC等於世界被清空，城市選單先收起來
      });
      $('cf-market-world').addEventListener('change', async function () {
        const w = $('cf-market-world').value;
        await MarketData.setWorld(w);
        await refreshCityOptions(w, null);
      });
      $('cf-market-city').addEventListener('change', function () {
        MarketData.setSellCity($('cf-market-city').value || null);
      });
  ...
  async function refreshWorldOptions(dcName, presetWorld) {
    ...
    worldSel.innerHTML = '<option value="">我的世界（賣出用）⋯</option>' +
  ...
  async function refreshCityOptions(worldNameStr, presetCityKey) {
    ...
    if (!worldNameStr) { citySel.style.display = 'none'; citySel.innerHTML = ''; return; }
    const cities = await MarketData.listSellCities(worldNameStr);
    if (!cities.length) { citySel.style.display = 'none'; citySel.innerHTML = ''; return; }
    citySel.innerHTML = '<option value="">賣出城市：自動（目前最低稅率）</option>' +
  ...
  function openWorldPopover(btn, itemId) {
    ...
    if (!pop || !breakdown.length) return;
    ...
    $('cf-batch-materials').innerHTML =
      ...
      (rows.map(function (r) {
        ...
        const toggle = r.craftable
          ? '<button type="button" class="craft-copy-btn" data-expand-toggle="' + r.itemId + '">自己做</button><span class="craft-mat-decision-hint" data-decision-item="' + r.itemId + '"></span>'
