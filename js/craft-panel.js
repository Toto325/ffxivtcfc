...
app._craftReady = function () {
            ...
            '<div class="craft-ingredients"><p class="craft-ingredients-title">整批訂單規劃　<span class="craft-muted">材料彙總／購物清單，可選「自己做」往下展開，或維持採購</span></p>' +
              ...
              '<div class="craft-row" style="gap:6px;margin-bottom:6px;justify-content:space-between;flex-wrap:wrap">' +
                ...
                '<select id="cf-batch-sort" class="craft-select" style="font-size:11px;min-width:0;flex:0 0 auto">' +
                  ...
                  '<option value="id">排序：物品ID</option>' +
                '</select>' +
              '</div>' +
  ...
  let batchSortMode = 'amount'; // 'amount'(預設，數量多到少) | 'name' | 'id' | 'cost'（需要已經查過價，沒查到就退回用amount）
  ...
  function refreshMarketDependentDisplays() {
    ...
    if (lastBatchRows) fillBatchCosts(lastBatchRows);
  }
        ...
        if (ings) {
          ...
          if (allResolved && craftCost > 0) {
            if (craftCost < r.totalCost) {
              html += ' <span class="craft-mat-decision craft-mat-decision-craft">自己做省' + Math.round(r.totalCost - craftCost).toLocaleString() + '金</span>';
            } else {
              html += ' <span class="craft-mat-decision craft-mat-decision-buy">買比較划算，省' + Math.round(craftCost - r.totalCost).toLocaleString() + '金</span>';
            }
  ...
  function fillBatchCosts(rows) {
    ...
    computeBatchFinancials(rows).then(function (fin) {
      ...
      if (Object.keys(combinedSubNeed).length) {
        MarketData.resolveBuyCosts(combinedSubNeed).then(function (combinedResults) {
          craftableRows.forEach(function (r) {
            ...
            const hintEl = document.querySelector('[data-decision-item="' + r.itemId + '"]');
            if (!hintEl) return;
            if (craftCost < buyEntry.totalCost) {
              hintEl.innerHTML = '<span class="craft-mat-decision craft-mat-decision-craft">省' + Math.round(buyEntry.totalCost - craftCost).toLocaleString() + '金</span>';
            } else {
              hintEl.innerHTML = '<span class="craft-mat-decision craft-mat-decision-buy">買划算，省' + Math.round(craftCost - buyEntry.totalCost).toLocaleString() + '金</span>';
            }
          });
      ...
      renderByWorldView(results, nameByItem);
    }).catch(function () {
  ...
  function renderByWorldView(results, nameByItem) {
    ...
    if (!worldNames.length) { box.innerHTML = '<p class="craft-muted">批次是空的</p>'; return; }
    box.innerHTML = worldNames.map(function (w) {
    ...
    Promise.all([costPromise, sellPromise, taxPromise, velocityPromise]).then(function (out) {
      ...
      if (sellRef) {
        ...
        parts.push('預估淨利 <strong style="color:' + (profit >= 0 ? '#4ade80' : '#f87171') + '">' + (profit >= 0 ? '+' : '') + Math.round(profit).toLocaleString() + '金</strong>' +
          '<span style="opacity:.7">／件（僅單件製作，不含製作時間成本）</span>');
      } else if (!settings.worldName) {
        parts.push('<span style="opacity:.7">未設定「我的世界」，看不到賣出參考與淨利</span>');
      } else {
        // 世界其實已經設定好了，只是這個物品目前該世界查不到掛單（可能沒人在賣，或剛好賣光），
        // 不能跟「根本沒設定世界」用同一句話講，不然玩家會誤以為是自己忘了設定。
        parts.push('<span style="opacity:.7">' + settings.worldName + '目前查無掛單（可能沒有人在賣），看不到賣出參考與淨利</span>');
      }
      if (velocity && velocity.totalVelocityPerDay) {
        // usedScope是'world'才是玩家自己那個世界的真實速度；退回'dc'代表混了整個資料中心其他世界的成交，僅供參考
        const scopeNote = velocity.usedScope === 'world' ? '' : '<span style="opacity:.7">（資料中心整體，未鎖定單一世界，僅供參考）</span>';
        parts.push('<i class="ph ph-chart-line"></i>近4天平均賣出約 <strong>' + velocity.totalVelocityPerDay.toFixed(1) + ' 件/天</strong>' + scopeNote);
      } else if (velocity && velocity.usedScope === null) {
        parts.push('<span style="opacity:.7">近4天查無成交紀錄，可能較冷門</span>');
      }
