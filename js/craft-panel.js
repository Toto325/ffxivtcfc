...
app._craftReady = function () {
  ...
  async function computeBatchFinancials(rows) {
    ...
    const sellPromise = settings.worldName
      ? Promise.all(sellItemIds.map(function (id) {
          return MarketData.getSellPrice(id).then(function (ref) { return { id: id, ref: ref }; });
        }))
      : Promise.resolve([]);
    ...
    const results = out[0], sellList = out[1], taxInfo = out[2];
    ...
    const sellByItem = {};
    sellList.forEach(function (s) { sellByItem[s.id] = s.ref; });
