...
var SPECIAL_ACTIONS = [
  ...
  /* 宇宙穩手（StellarSteadyHand，遊戲內ID 46843）目前繁中版本語言表沒有這個技能的翻譯，
   * 代表這是還沒實裝到繁中服的後期版本技能。求解器不能預設玩家擁有這種技能，
   * 所以跟其他「需要另外確認」的技能一樣放進這裡，但預設「不勾選」（defaultOff），
   * 跟其他預設「有/想用」的技能不同——避免求解結果算出一套繁中服玩家根本用不出來的手法。
   * 如果之後繁中服跟上版本、這個技能語言表補上了，把 defaultOff 拿掉即可恢復成一般判定。 */
  { key: 'StellarSteadyHand', label: '宇宙穩手', note: '繁中服尚未實裝此技能（後期版本才有）', defaultOff: true },
];
...
app._craftReady = function () {
  ...
  function materialNodeHtml(itemId, direction, seenPath, knownRecipeId) {
    ...
    return '<div class="craft-mat-node" data-item="' + itemId + '" data-dir="' + direction + '" data-path="' + seenPath + '">' +
      '<div class="craft-mat-row">' +
        ...
        (craftableRecipeId
          ? '<span class="craft-mat-name craft-mat-name-clickable" data-switch-recipe="' + craftableRecipeId + '" title="點一下切換成求解對象">' + itemIconHtml(itemId, 18) + itemName(itemId) + '</span>'
  ...
  function bindMatActions(container) {
    ...
    container.querySelectorAll('.craft-mat-switch, .craft-mat-name-clickable').forEach(function (btn) {
      btn.addEventListener('click', function (e) {
        e.stopPropagation();
