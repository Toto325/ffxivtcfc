...
app._craftReady = function () {
        ...
        '<div class="craft-explore-body">' +
          ...
          '<div id="cf-explore-batch" class="craft-explore-pane" style="display:none">' +
            ...
            '<div class="craft-ingredients"><p class="craft-ingredients-title">整批訂單規劃　<span class="craft-muted">材料彙總／購物清單，可選「自己做」往下展開，或維持採購</span></p>' +
              ...
              '<div id="cf-batch-materials"></div></div>' +
          '</div>' +
        '</div>' +
      '</div>' +
    '</div>';
  ...
  function ensureMarketSettingsPopoverDom() {
    ...
    pop.innerHTML =
      ...
      '<div class="craft-settings-field"><label>資料中心<span class="craft-muted">（材料成本查詢範圍）</span></label><select id="cf-market-dc" class="craft-select-block"><option value="">資料中心⋯</option></select></div>' +
      '<div class="craft-settings-field"><label>我的世界<span class="craft-muted">（賣出參考只看這裡）</span></label><select id="cf-market-world" class="craft-select-block"><option value="">我的世界⋯</option></select></div>' +
      '<div class="craft-settings-field" id="cf-market-city-field" style="display:none"><label>賣出城市<span class="craft-muted">（決定賣方稅率）</span></label><select id="cf-market-city" class="craft-select-block"></select></div>';
  ...
  function openWorldPopover(btn, itemId) {
    ...
    pop.innerHTML = '<p class="craft-mat-worlds-title">分' + breakdown.length + '個世界湊齊，由便宜到貴：</p>' +
      breakdown.map(function (b) {
        return '<div class="craft-mat-world-row"><span class="w-name">' + b.world + '</span><span class="w-qty">×' + b.qty.toLocaleString() + '</span><span class="w-price">' + b.unitPrice.toLocaleString() + '金/個</span></div>';
      }).join('');
    pop.style.display = 'flex';
  ...
  function materialNodeHtml(itemId, direction, seenPath, knownRecipeId) {
    ...
    // 價格／決策標籤長度不固定（有時候只是「1,200金／伊弗利特」，有時候還要多塞「自己做省12,534金」），
    // 之前把它塞在同一行的flex裡，字數一多就會不規則換行，每一列高度都不一樣、看起來像跑版。
    // 改成價格永遠獨立一行放在名稱下面，不管內容多長，都只影響「這一列」自己的高度，不會波及排版對齊。
    return '<div class="craft-mat-node" data-item="' + itemId + '" data-dir="' + direction + '" data-path="' + seenPath + '">' +
      '<div class="craft-mat-row">' +
        ...
        '</div>' +
      '</div>' +
      '<div class="craft-mat-price-row"><span class="craft-mat-price" data-price-item="' + itemId + '"' + (craftableRecipeId ? ' data-craft-recipe="' + craftableRecipeId + '"' : '') + '></span></div>' +
  ...
  function renderBatch() {
    ...
    $('cf-batch-materials').innerHTML =
      ...
      (rows.map(function (r) {
        ...
        // 跟材料圖譜節點同樣的道理：價格＋決策標籤長度不固定，獨立成第二行，不要跟名稱/按鈕擠在同一行裡，
        // 不然決策標籤一長就會把整列擠得七零八落、每列高度長短不一。
        return '<div class="craft-mat-buy-block" data-buy-item="' + r.itemId + '">' +
          '<div class="craft-ing-row" data-buy-qty="' + r.amount + '"><span class="craft-ing-name">' + r.name + '</span><span class="craft-row" style="gap:8px"><span class="craft-muted">×' + r.amount.toLocaleString() + '</span>' +
          (rid ? '<button type="button" class="craft-mat-switch" data-switch-recipe="' + rid + '" title="切換成求解對象">求解</button>' : '') +
          toggleBtn + '</span></div>' +
          '<div class="craft-mat-price-row-batch"><span class="craft-mat-price" data-buy-cost-item="' + r.itemId + '"></span>' +
          (r.craftable ? '<span class="craft-mat-decision-hint" data-decision-item="' + r.itemId + '"></span>' : '') + '</div>' +
          '</div>';
