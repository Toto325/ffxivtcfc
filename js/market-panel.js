...
(function () {
   ...
   * 第4點修正：NQ/HQ不能只靠顏色分（色盲使用者分不出來），改成「形狀」當主要區分依據——
   * NQ是圓點、HQ是菱形，顏色只是輔助強化，就算完全看不出顏色差異，形狀本身就能分辨。 */
  ...
  function ensureTrendModalDom() {
    ...
    modal.innerHTML = '<div class="market-modal-box market-modal-box-wide">' +
      ...
      '<div id="mk-trend-body" class="market-modal-body"></div>' +
    '</div>';
    document.body.appendChild(modal);
    modal.addEventListener('click', function (e) { if (e.target === modal) modal.style.display = 'none'; });
  ...
  function openTrendModal(history) {
    ...
    body.innerHTML = (history && history.length >= 2) ? renderTrendChart(history) : '<p class="craft-muted">近期成交紀錄不足，無法畫出走勢。</p>';
    modal.style.display = 'flex';
  }
