...
(function () {
  ...
  async function loadMarketSection(itemId) {
    ...
    // 第2點：走勢圖改放到供應鏈那一欄（mk-detail-trend），不要再擠在單價欄上面——
    // 單價欄的優先順序是「盡量多顯示掛單筆數」，走勢圖挪去空間本來就比較寬鬆的那一側。
    const trendBox = document.getElementById('mk-detail-trend');
    if (trendBox) trendBox.innerHTML = (overview && overview.history && overview.history.length >= 2) ? renderTrendChart(overview.history) : '';
    ...
    html += '<p class="market-subheading">目前掛單（共 ' + overview.listings.length + ' 筆） ' +
      ...
      overview.listings.map(function (l) {
        ...
        return '<tr><td>' + l.pricePerUnit.toLocaleString() + '金' + (l.hq ? ' ' + hqIconHtml() : '') + '</td><td>' + l.quantity.toLocaleString() + '</td><td>' + l.world + '</td></tr>';
  ...
  function renderSupplyChain(itemId, rid, box) {
    ...
    const bendY2 = centerY + centerR + 40; // 比原本多留一點空間放中心物品的名稱標籤（第8點）
    ...
    const centerFit = fitName(String(centerName), Math.min(W - 40, 260), 12.5, 9);
    box.innerHTML =
      '<svg viewBox="0 0 ' + W + ' ' + H + '" class="market-supply-svg"><defs>' + svgDefs + '</defs>' +
        ...
        '</g>' + centerFit.titleAttr +
        '<text x="' + cx + '" y="' + (centerY + centerR + 20) + '" text-anchor="middle" font-size="' + centerFit.fontSize.toFixed(1) + '" fill="#fcf6ba" font-weight="600">' + centerFit.text + '</text>' +
      '</svg>' +
  ...
  function renderRadarRows(jobId, rows, fromCache) {
    ...
    body.innerHTML =
      ...
      '<div class="market-table-scroll"><table class="market-price-table"><thead><tr><th>淨利／次</th><th>材料成本</th><th>真實最低價／次<span class="craft-muted" style="font-weight:normal">（全服）</span></th><th>物品</th></tr></thead><tbody>' +
      rows.map(function (r) {
      ...
      '<p class="craft-muted" style="margin-top:6px">粗估值：只算直接材料成本，沒有考慮製作時間跟賣方稅；「淨利／次」是做一次配方（可能一次做出不只1件）的總損益，不是單件價格。「真實最低價」是整個資料中心裡最便宜的掛單——買方能跨世界買，只看你自己世界的價格會漏掉「其實有人在別的世界用更低價賣」這件事，所以拿全服最低價當能不能賺的判斷基準；你自己世界的價格另外列出來當參考，那才是你實際上架時看得到的競爭對手。</p>';
