...
(function () {
  ...
  function fitMarketHeights() {
    ...
    const viewportCap = window.innerHeight * 0.82;
  ...
  function openCraftSolverModal(rid) {
    ...
    // 第5點抓到的原因：只加market-modal-scene這個class，用!important蓋display是蓋得過去，
    // 但生產頁面本身其他跟版面配置有關的程式碼／樣式很可能還是照著「有沒有.active」這個class
    // 在判斷要不要正常運作，光靠我自訂的class蓋display，蓋得到「看不看得見」，蓋不到「其他跟著
    // .active走的行為」，才會看起來完全沒反應。這裡把.active一併加上，等於讓瀏覽器認為這個
    // scene「正常被啟用了」，只是額外套了一層彈窗樣式蓋掉原本滿版的視覺呈現方式。
    craftSection.classList.add('market-modal-scene', 'active');
    if (!craftSection.querySelector('.market-modal-scene-close')) {
      const closeBtn = document.createElement('button');
      closeBtn.type = 'button';
      closeBtn.className = 'market-modal-scene-close';
      closeBtn.innerHTML = '<i class="ph ph-x"></i>';
      closeBtn.addEventListener('click', function () { craftSection.classList.remove('market-modal-scene', 'active'); });
      craftSection.appendChild(closeBtn);
    }
    if (typeof app !== 'undefined' && typeof app.openCraftForRecipe === 'function') app.openCraftForRecipe(rid);
  }
  ...
  function renderTrendChart(rawPts) {
    ...
    const ordered = rawPts.slice().reverse(); // 由舊到新，由左到右畫
    ...
    const withXY = ordered.map(function (h, i) { return Object.assign({}, h, xy(i, h.pricePerUnit)); });
    function buildLine(list) {
      return list.map(function (p, i) { return (i === 0 ? 'M' : 'L') + p.x.toFixed(1) + ',' + p.y.toFixed(1); }).join(' ');
    }
    const nqLine = buildLine(withXY.filter(function (p) { return !p.hq; }));
    const hqLine = buildLine(withXY.filter(function (p) { return p.hq; }));
    const dots = withXY.map(function (h) {
      ...
      if (h.hq) {
        const cx = h.x, cy = h.y, rOuter = 6, rInner = 2.6;
        let starPts = '';
        for (let k = 0; k < 10; k++) {
      ...
      return '<circle cx="' + h.x.toFixed(1) + '" cy="' + h.y.toFixed(1) + '" r="3.2" fill="#8fb3ff" stroke="#1a2a3a" stroke-width="0.8"><title>' + label + '</title></circle>';
    }).join('');
    ...
    return '<svg viewBox="0 0 ' + W + ' ' + H + '" preserveAspectRatio="none" class="market-trend-svg">' +
        ...
        '<path d="' + nqLine + '" fill="none" stroke="#8fb3ff" stroke-width="1.5" opacity="0.85"/>' +
        '<path d="' + hqLine + '" fill="none" stroke="#fcf6ba" stroke-width="1.5" stroke-dasharray="5,3" opacity="0.85"/>' +
        dots +
      '</svg>' +
      '<p class="craft-muted" style="font-size:10px;margin-top:4px"><span style="color:#8fb3ff">──</span> NQ（實線）　<span style="color:#fcf6ba">┅┅</span> HQ（虛線）</p>';
  }
  ...
  async function openAvgPricePopover(itemId, anchorBtn, fallbackAvg) {
    const pop = ensureAvgPricePopoverDom();
    pop.innerHTML = '<p class="craft-mat-worlds-title">均價比較</p><p class="craft-muted" style="font-size:11px">讀取歷史成交中⋯</p>';
    ...
    try {
      ...
      const oneDayAgo = avgInRange(24, 48);
      ...
      pop.innerHTML = '<p class="craft-mat-worlds-title">均價比較</p>' +
        '<p class="market-avgprice-row"><span>當前（近24小時成交）</span><strong>' + (current != null ? current.toLocaleString() + ' 金' : '無成交') + '</strong></p>' +
        '<p class="market-avgprice-row"><span>1天前（24~48小時前）</span><strong>' + (oneDayAgo != null ? oneDayAgo.toLocaleString() + ' 金' : '無成交') + '</strong></p>' +
        ...
        '<p class="craft-muted" style="font-size:10px;margin-top:4px">「當前」「1天前」是單獨那24小時的均價；「近3/7/30天」是過去那整段時間所有成交一起算的平均，兩種算法不一樣，跨度越寬的樣本數越多。</p>';
    } catch (e) {
      pop.innerHTML = '<p class="craft-mat-worlds-title">均價比較</p><p class="craft-muted" style="font-size:11px">讀取失敗，請稍後再試。</p>';
    }
  }
  ...
  async function loadMarketSection(itemId) {
    ...
    // 第1點：均價改成「近24小時實際成交均價」，不是掛單快照的均價——這樣均價才是「最近真的賣多少錢」，
    // 不是「現在架上還沒賣掉的東西平均開價多少」，兩者概念不同，後者容易被囤積的高價/低價掛單誤導。
    // 24小時內沒有成交紀錄的冷門物品，退回用掛單快照均價，並註明這是退回值，不是真的成交均價。
    const last24h = (overview && overview.history) ? overview.history.filter(function (h) { return h.timestamp >= Date.now() / 1000 - 86400; }) : [];
    const avg24h = last24h.length ? Math.round(last24h.reduce(function (s, h) { return s + h.pricePerUnit; }, 0) / last24h.length) : null;
    const avgIsFallback = avg24h == null && overview && overview.avgPrice;
    const avgDisplay = avg24h != null ? avg24h : (overview && overview.avgPrice ? Math.round(overview.avgPrice) : null);
    if (avgDisplay != null) badgeParts.push('<button type="button" class="market-stat-badge market-stat-badge-btn" id="mk-avgprice-badge">均價 ' + avgDisplay.toLocaleString() + '金' + (avgIsFallback ? '<span class="craft-muted">＊</span>' : '') + '<i class="ph ph-caret-down" style="font-size:9px;margin-left:3px"></i></button>');
    ...
    if (trendBtn) {
      trendBtn.addEventListener('click', function () { openTrendModal(itemId); });
    }
    ...
    if (avgBtn) {
      avgBtn.addEventListener('click', function (e) {
        ...
        openAvgPricePopover(itemId, avgBtn, overview.avgPrice);
      });
    }

    // hq-only／世界篩選／手續費都是純畫面篩選或計算（世界篩選除外，那個要重新查，見下面說明）
    let hqOnly = false, includeFee = false, worldFilter = 'ALL';
    ...
    $('mk-world-filter').addEventListener('change', async function () {
      ...
      if (worldFilter === 'ALL') { activeListingSet = overview.listings; renderListingTableFromActiveSet(); return; }
      if (worldListingsCache[worldFilter]) { activeListingSet = worldListingsCache[worldFilter]; renderListingTableFromActiveSet(); return; }
  ...
  function openTrendModal(itemId) {
    ...
    async function renderForRange() {
      ...
      if (cache[activeRange]) { $('mk-trend-svg-slot').innerHTML = renderTrendChart(cache[activeRange]); return; }
      $('mk-trend-svg-slot').innerHTML = '<p class="craft-muted" style="padding:12px 0">讀取中，成交熱絡的道具可能需要翻好幾頁資料，請稍候⋯</p>';
      try {
        // 第6點：不再用單次回應裡那份被上限截斷的資料，改用會自動翻頁的fetchFullHistory，
        // 確保30天真的抓得到30天，不會因為單次回應被塞滿就默默縮水成只剩最近幾天。
        const full = await MarketData.fetchFullHistory(itemId, rangeDef.days);
        const filtered = full.filter(function (h) { return h.timestamp >= (Date.now() / 1000) - rangeDef.days * 86400; });
        cache[activeRange] = filtered;
        $('mk-trend-svg-slot').innerHTML = renderTrendChart(filtered);
      } catch (e) {
        $('mk-trend-svg-slot').innerHTML = '<p class="craft-muted" style="padding:12px 0">讀取失敗，請稍後再試一次。</p>';
      }
    }
    $('mk-trend-range-row').querySelectorAll('[data-range]').forEach(function (b) {
  ...
  function renderRadarShell() {
    ...
    box.innerHTML =
      ...
      '</div>' +
      '<div id="mk-radar-body" class="market-pane-scrollbody"><p class="craft-muted">選一個職業開始掃描，找出目前成本低、賣得快的配方。</p></div>';
  ...
  const HOT_METRICS = [
    ...
    { key: 'changePct', label: '漲跌幅度', fmt: function (v) { return (v >= 0 ? '+' : '') + v.toFixed(1) + '%'; }, hint: '短期動能：近3天成交均價，比近7天成交均價貴/便宜多少（可切換成近1天比近3天）' },
  ];
  ...
  async function renderHotShell() {
    ...
    box.innerHTML =
      '<div class="market-hot-controls market-pane-fixed">' +
        ...
        '<div class="market-hot-metrics">' +
          ...
          '<span class="market-hot-mode-group" id="mk-hot-mode-group" style="display:none">' +
            '<button type="button" class="craft-job-filter-btn active" data-mk-hot-mode="3v7">近3天比近7天</button>' +
            '<button type="button" class="craft-job-filter-btn" data-mk-hot-mode="1v3">近1天比近3天</button>' +
          '</span>' +
        '</div>' +
      '</div>' +
      '<div id="mk-hot-body" class="market-pane-scrollbody"><p class="craft-muted">選好範圍跟指標後，按下面的按鈕開始掃描目前市場上最熱門的道具。</p><button type="button" class="market-fullchain-btn" id="mk-hot-start">開始掃描</button></div>';
    fitMarketHeights();

    let scope = 'ALL', metric = 'velocity', changeMode = '3v7';
    box.querySelectorAll('[data-mk-hot-scope]').forEach(function (btn) {
      btn.addEventListener('click', function () {
        ...
        runHotScan(scope, metric, changeMode);
      });
    });
    box.querySelectorAll('[data-mk-hot-metric]').forEach(function (btn) {
      btn.addEventListener('click', function () {
        ...
        $('mk-hot-mode-group').style.display = metric === 'changePct' ? 'inline-flex' : 'none';
        renderHotFromCacheOrRescan(scope, metric, changeMode);
      });
    });
    box.querySelectorAll('[data-mk-hot-mode]').forEach(function (btn) {
      btn.addEventListener('click', function () {
        box.querySelectorAll('[data-mk-hot-mode]').forEach(function (b) { b.classList.remove('active'); });
        btn.classList.add('active');
        changeMode = btn.dataset.mkHotMode;
        localStorage.removeItem(HOT_CACHE_KEY); // 換比較區間，快取裡的漲跌幅度是舊區間算出來的，強制重新掃描
        runHotScan(scope, metric, changeMode);
      });
    });
  ...
  function renderHotFromCacheOrRescan(scope, metric, changeMode) {
    const cached = loadHotCache(scope);
    if (cached) renderHotRows(cached, metric, true, scope);
    else runHotScan(scope, metric, changeMode);
  }

  async function runHotScan(scope, metric, changeMode) {
    ...
    try {
      ...
      // 第1點抓到的真正原因：這支端點是「最近更新的事件」清單，不是「不重複的物品」清單——
      // 同一個熱門物品短時間內被更新好幾次，就會在原始清單裡出現好幾筆，沒濾重複才會排行榜
      // 同一個物品佔好幾個名次、數字還一模一樣。這裡用Set濾掉重複的itemId，保留第一次出現的順序。
      const seen = {};
      ...
      const out = await Promise.all([
        ...
        MarketData.fetchHistoryBatch(numericIds, scopeName, changeMode),
        MarketData.fetchListingsBatch(numericIds), // 第10點：DC全服掛單一次抓齊，「所有世界」跟單一世界都從這份資料篩選就好，不用分開查
      ]);
      const velResult = out[0], histResult = out[1], listingsResult = out[2];
      const rows = candidateIds.map(function (id) {
        const vel = velResult[id] || {};
        const hist = histResult[id] || {};
        const listings = listingsResult[id] || [];
        // 第10點：選「所有世界」看全服最低，選特定世界就只看那個世界自己的掛單
        const relevant = scope === 'ALL' ? listings : listings.filter(function (l) { return l.worldName === scope; });
        const minPrice = relevant.length ? relevant[0].pricePerUnit : null;
        return {
          itemId: id, name: ITEM_NAMES_TW_ALL[id] || ('#' + id), minPrice: minPrice,
          stats: {
  ...
  function renderHotRows(rows, metricKey, fromCache, scope) {
    ...
    if (!top.length) {
      body.innerHTML = '<p class="craft-muted">目前這個指標查不到足夠的資料，換個指標或範圍試試。 <button type="button" class="market-history-btn" id="mk-hot-refresh-empty">重新整理</button></p>';
      const rb = $('mk-hot-refresh-empty');
      if (rb) rb.addEventListener('click', function () { localStorage.removeItem(HOT_CACHE_KEY); runHotScan(scope, metricKey); });
      return;
    }
    ...
    body.innerHTML =
      ...
      '<div class="market-hot-list">' +
        top.map(function (r, i) {
          ...
          // 第7點：開根號比例在數值差距非常懸殊時（例如8萬 vs 250~450，約200倍差距）還是不夠，
          // 全部會被壓在很小的一段區間裡，肉眼看起來還是差不多長。改用對數比例——這是處理
          // 跨好幾個數量級的長尾分佈資料時更標準的做法，同樣的例子換算後會分布在50%上下，
          // 不會全部擠在個位數百分比裡分不出差異。
