...
(function () {
  ...
  async function loadMarketSection(itemId) {
     ...
     * 賣速徽章不受影響（它有自己的範圍標示）。 */
    const velBadge = (function () {
      if (velocity && (velocity.nqVelocityPerDay != null || velocity.hqVelocityPerDay != null)) {
        const scopeNote = velocity.usedScope === 'world' ? '' : '（DC）';
        return '<span class="market-stat-badge">賣速 NQ ' + (velocity.nqVelocityPerDay || 0).toFixed(1) + '/天　HQ ' + (velocity.hqVelocityPerDay || 0).toFixed(1) + '/天' + scopeNote + '</span>';
      }
      if (velocity && velocity.usedScope === null) return '<span class="market-stat-badge market-stat-badge-muted">近4天查無成交</span>';
      return '';
    })();
    function buildStatBarHtml(st) {
      ...
      if (velBadge) parts.push(velBadge);
      return parts.join('');
    }
    ...
    html += '<p class="market-subheading">目前掛單（共 ' + overview.listings.length + ' 筆） ' +
        ...
        '<select id="mk-world-filter" class="craft-select" style="margin-left:6px;font-size:11px">' +
          // optgroup 的標題列看起來像分類、不能被選取，只當說明用；預設選中的仍是裡面第一個「所有世界」
          '<optgroup label="掛單、均價及走勢圖顯示範圍">' +
    ...
    async function updateStatBar() {
      ...
      setStatBar(Object.assign({ avgLoading: true }, base));
      let hist = worldHist24Cache[worldFilter];
      if (!hist) {
        try { hist = await MarketData.fetchFullHistory(itemId, 1, worldFilter); worldHist24Cache[worldFilter] = hist; }
        catch (e) { hist = []; }
      }
  ...
  function openTrendModal(itemId, scopeWorld) {
    ...
    async function renderForRange() {
      ...
      try {
        ...
        const full = await MarketData.fetchFullHistory(itemId, rangeDef.days, scopeWorld || undefined);
  ...
  const HOT_CACHE_VERSION = 3; // 資料結構改版（新增樣本數／NQHQ分開算）時遞增，舊快取自動作廢
  const HOT_METRICS = [
    { key: 'velocity', label: '賣速', fmt: function (v) { return v.toFixed(1) + ' 件/天'; }, hint: '流動性：多快能賣掉，數字越高代表越搶手（NQ+HQ合計，跟物品詳情頁同一套計算方式）' },
    { key: 'changePct', label: '漲跌幅度', fmt: function (v) { return (v >= 0 ? '+' : '') + v.toFixed(1) + '%'; }, hint: '短期動能：當前（1~24小時）成交均價，比近1天（1~48小時）成交均價貴/便宜多少（NQ／HQ一起算，成交筆數不足的不列入）' },
  ];
  ...
  async function renderHotShell() {
    ...
    box.innerHTML =
      '<div class="market-hot-controls market-pane-fixed">' +
        ...
        '</div>' +
        '<p class="craft-muted market-hot-scope-note">ℹ 玩家只能從自己所屬的世界掛賣，「所有世界」的數字僅供參考，請優先選擇自己所屬的世界。</p>' +
  ...
  function renderHotRows(rows, metricKey, fromCache, scope, opts) {
    ...
    body.innerHTML =
      ...
      (metricKey === 'changePct' ? '漲跌算法：當前（1~24小時）成交均價 vs 近1天（1~48小時）成交均價，NQ／HQ 不分開，所有成交紀錄一起平均；成交太密、資料只涵蓋不到48小時的道具會等比例縮短窗口（標「約Nh」）。' : '') + '</p>';
