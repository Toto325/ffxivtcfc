...
(function () {
  ...
  const HOT_CACHE_VERSION = 2; // 資料結構改版（新增樣本數／NQHQ分開算）時遞增，舊快取自動作廢
  const HOT_METRICS = [
    { key: 'velocity', label: '賣速', fmt: function (v) { return v.toFixed(1) + ' 件/天'; }, hint: '流動性：多快能賣掉，數字越高代表越搶手（NQ+HQ合計，跟物品詳情頁同一套計算方式）' },
    { key: 'changePct', label: '漲跌幅度', fmt: function (v) { return (v >= 0 ? '+' : '') + v.toFixed(1) + '%'; }, hint: '短期動能：當前（1~24小時）成交均價，比近1天（1~48小時）成交均價貴/便宜多少（NQ／HQ分開算，成交筆數不足的不列入）' },
  ];
  ...
  function renderHotRows(rows, metricKey, fromCache, scope, opts) {
    ...
    body.innerHTML =
      ...
      '<div class="market-hot-list">' +
        top.map(function (r, i) {
          ...
          if (metricKey === 'velocity' && (r.stats.nqVelocity != null || r.stats.hqVelocity != null)) {
            ...
            subLine = '<span class="market-hot-sub">' + ci.quality + ' ' + Math.round(ci.longAvg).toLocaleString() + '→' + Math.round(ci.shortAvg).toLocaleString() +
      ...
      (metricKey === 'changePct' ? '漲跌算法：當前（1~24小時）成交均價 vs 近1天（1~48小時）成交均價，NQ／HQ 分開算，取成交筆數較多的一邊；成交太密、資料只涵蓋不到48小時的道具會等比例縮短窗口（標「約Nh」）。' : '') + '</p>';
