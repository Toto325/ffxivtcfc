...
(function () {
  ...
  function fitMarketHeights() {
    ...
    const topRect = topFixed.getBoundingClientRect();
    const available = Math.max(160, Math.floor(rootRect.bottom - topRect.bottom));
  ...
  function openItemDetail(itemId) {
    ...
    $('mk-pane-item').innerHTML =
      '<div class="market-detail-header">' + itemIconHtml(itemId, 40) + '<h3>' + name + '</h3>' +
        ...
        '<button type="button" class="market-history-btn" id="mk-solve-btn" disabled>查詢是否可製作⋯</button>' +
      '</div>' +
  ...
  async function bindSolveButton(itemId) {
    ...
    if (typeof CRAFT_RECIPES === 'undefined') { btn.textContent = '無法製作'; return; }
    const rid = (buildToRecipesIndex()[itemId] || [])[0];
    if (!rid) { btn.textContent = '無法製作'; return; }
    btn.textContent = '製作求解';
    btn.disabled = false;
    btn.addEventListener('click', function () {
      window.location.hash = 'recipe=' + rid;
      if (window.app && typeof window.app.nav === 'function') window.app.nav('craft');
    });
  }
  ...
  async function openAvgPricePopover(itemId, anchorBtn, currentAvg) {
    ...
    try {
      ...
      pop.innerHTML = '<p class="craft-mat-worlds-title">均價比較</p>' +
        '<p class="market-avgprice-row"><span>目前掛單均價</span><strong>' + Math.round(currentAvg).toLocaleString() + ' 金</strong></p>' +
        offsets.map(function (d) {
          const v = avgWithin(d);
          return '<p class="market-avgprice-row"><span>近' + d + '天成交均價</span><strong>' + (v != null ? v.toLocaleString() + ' 金' : '無資料') + '</strong></p>';
        }).join('') +
        '<p class="craft-muted" style="font-size:10px;margin-top:4px">「目前掛單均價」是現在正在賣的價格平均，「近N天成交均價」是實際成交紀錄的平均，兩者概念不同。</p>';
    } catch (e) {
  ...
  async function loadMarketSection(itemId) {
    ...
    if (overview && overview.avgPrice) badgeParts.push('<button type="button" class="market-stat-badge market-stat-badge-btn" id="mk-avgprice-badge">均價 ' + Math.round(overview.avgPrice).toLocaleString() + '金<i class="ph ph-caret-down" style="font-size:9px;margin-left:3px"></i></button>');
    ...
    if (avgBtn) {
      avgBtn.addEventListener('click', function () { openAvgPricePopover(itemId, avgBtn, overview.avgPrice); });
    }
  ...
  function ensureHistoryModalDom() {
    ...
    modal.innerHTML = '<div class="market-modal-box">' +
      ...
      '<div class="market-flexrow market-flexrow-head">' +
        '<span class="mfr-price">成交價</span><span class="mfr-qty">數量</span><span class="mfr-world">世界</span><span class="mfr-time">時間</span>' +
      '</div>' +
  ...
  function openHistoryModal(itemId, history) {
    ...
    if (!history || !history.length) {
      ...
      body.innerHTML = history.slice(0, 50).map(function (h) {
        return '<div class="market-flexrow"><span class="mfr-price">' + h.pricePerUnit.toLocaleString() + '金' + (h.hq ? ' ' + hqIconHtml() : '') + '</span><span class="mfr-qty">' + h.quantity.toLocaleString() + '</span><span class="mfr-world">' + h.world + '</span><span class="mfr-time craft-muted">' + timeAgo(h.timestamp) + '</span></div>';
      }).join('');
    }
  ...
  async function runHotScan(scope, metric) {
    ...
    try {
      ...
      const candidateIds = await MarketData.fetchMostRecentlyUpdated(scopeType, scopeName, 200);
