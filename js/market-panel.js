  ...
  async function loadMarketSection(itemId) {
    ...
    html += '<p class="market-subheading">目前掛單（共 ' + overview.listings.length + ' 筆） ' +
      ...
      overview.listings.map(function (l) {
        ...
        return '<tr><td>' + l.pricePerUnit.toLocaleString() + '金' + (l.hq ? ' <span class="market-hq-star" title="HQ">★</span>' : '') + '</td><td>' + l.quantity.toLocaleString() + '</td><td>' + l.world + '</td></tr>';
    ...
    if (!history || !history.length) {
      ...
      body.innerHTML = '<table class="market-price-table"><thead><tr><th>成交價</th><th>數量</th><th>世界</th><th>時間</th></tr></thead><tbody>' +
        history.slice(0, 50).map(function (h) {
          return '<tr><td>' + h.pricePerUnit.toLocaleString() + '金' + (h.hq ? ' <span class="market-hq-star" title="HQ">★</span>' : '') + '</td><td>' + h.quantity.toLocaleString() + '</td><td>' + h.world + '</td><td class="craft-muted">' + timeAgo(h.timestamp) + '</td></tr>';
        }).join('') + '</tbody></table>';
    }
