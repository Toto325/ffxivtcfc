...
(function () {
  ...
  async function openAvgPricePopover(itemId, anchorBtn, fallbackAvg) {
    ...
    try {
      ...
      // 第7點：「近1天成交均價」照你的定義＝1~48小時（當天＋昨天）。
      ...
      pop.innerHTML = '<p class="craft-mat-worlds-title">均價比較</p>' +
        ...
        '<p class="craft-muted" style="font-size:10px;margin-top:4px">「當前」是最近24小時內的成交；「近1天」是1~48小時（當天＋昨天）；「近3/7/30天」是過去那整段時間所有成交的平均。</p>';
    } catch (e) {
  ...
  const HOT_METRICS = [
    ...
    { key: 'changePct', label: '漲跌幅度', fmt: function (v) { return (v >= 0 ? '+' : '') + v.toFixed(1) + '%'; }, hint: '短期動能：當前（1~24小時）成交均價，比近1天（1~48小時）成交均價貴/便宜多少' },
  ];
  ...
  function saveHotCache(scope, rows) {
    ...
    const key = scope + ':' + s.dcName;
    const payload = {};
    payload[key] = { time: Date.now(), rows: rows };
    // 第4點：快取之前會整份累積各個世界的結果，資料一多就可能超出localStorage容量上限，
    // 寫入失敗又被空的catch吞掉，所以看起來像「快取消失了」。改成只保留當前這個範圍的結果
    // （切換世界時舊的就丟掉），資料量小很多；萬一還是寫不下，先清空整份再寫一次。
    try {
  ...
  async function runHotScan(scope, metric) {
    ...
    try {
      ...
      /* 第3/6點的核心重構：以前是「200個候選全部都去查完整歷史紀錄＋掛單」，那是最慢的兩種
       * 查詢，200個都查當然要好幾分鐘；更糟的是只要某個物品的歷史查詢失敗，它就被整個濾掉不
       * 進排行，失敗一多排行就從幾十個縮水到剩不到10個，這就是你看到的「結果大幅下降」。
       * 改成兩階段：
       *  階段一：只查「賣速」——這支是Universalis已經算好的聚合統計，一次要200個也很快。
       *  階段二：先用賣速排出名次，只對「真正會顯示出來的前40名」去查歷史紀錄跟掛單價格。
       * 這樣重的查詢從200項降到40項，速度大幅改善，而且就算某項查失敗，也只是那一項少了
       * 漲跌或價格的數字，不會把整個物品從排行榜上除名。 */
      body.innerHTML = '<p class="craft-muted">掃描中，正在查 ' + numericIds.length + ' 項道具的賣速⋯</p>';
      const velResult = await MarketData.fetchSaleVelocityBatch(numericIds, scopeName);
      ...
      // 只保留有賣速的（真的有在流動的東西才叫「熱」），再取前40名去做第二階段的細查
      rows = rows.filter(function (r) { return r.stats.velocity > 0; })
                 .sort(function (a, b) { return b.stats.velocity - a.stats.velocity; })
                 .slice(0, 40);
      if (!rows.length) { body.innerHTML = '<p class="craft-muted">目前查不到有成交流動的道具，稍後再試試看。</p>'; return; }

      body.innerHTML = '<p class="craft-muted">已排出前 ' + rows.length + ' 名，正在查詳細價格與漲跌⋯</p>';
      const topIds = rows.map(function (r) { return Number(r.itemId); });
      const out = await Promise.all([
        MarketData.fetchHistoryBatch(topIds, scopeName).catch(function () { return {}; }),
        MarketData.fetchListingsBatch(topIds).catch(function () { return {}; }),
      ]);
      const histResult = out[0], listingsResult = out[1];
      rows.forEach(function (r) {
        const hist = histResult[r.itemId];
        if (hist) r.stats.changePct = hist.changePct;
        const listings = listingsResult[r.itemId] || [];
        // 第6點：選「所有世界」看全服最低，選特定世界就只看那個世界自己的掛單
        const relevant = scope === 'ALL' ? listings : listings.filter(function (l) { return l.worldName === scope; });
        if (relevant.length) r.minPrice = relevant[0].pricePerUnit;
      });
      /* 第6點：照你的建議做——批次查完還是沒拿到價格的，逐一單獨再查一次。因為第二階段只剩
       * 40項，真正缺價的通常只有幾項，單獨補查的成本很低，卻能繞開「批次回應太大導致某些物品
       * 被漏掉」這個問題（批次查不到就是查不到，重試整批也還是同樣結果，單獨查反而會成功）。 */
      const missing = rows.filter(function (r) { return r.minPrice == null; });
      if (missing.length) {
        body.innerHTML = '<p class="craft-muted">正在補查 ' + missing.length + ' 項道具的價格⋯</p>';
        await Promise.all(missing.map(async function (r) {
          try {
            const single = await MarketData.fetchListingsBatch([Number(r.itemId)]);
