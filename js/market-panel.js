...
(function () {
  ...
  function ensureCraftModalDom() {
    ...
    modal.innerHTML = '<div class="market-craft-modal-box">' +
      '<button type="button" class="market-modal-scene-close" data-mk-close-craft="1"><i class="ph ph-x"></i></button>' +
  ...
  function openCraftSolverModal(rid) {
    ...
    $('mk-craft-modal-iframe').src = window.location.pathname + '#page=craft&recipe=' + rid;
  ...
  async function openAvgPricePopover(itemId, anchorBtn, fallbackAvg) {
    ...
    try {
      ...
      // 第7點：「近1天成交均價」照你的定義＝1~48小時（當天＋昨天），不是只有最近24小時。
      // 「當前成交均價」則是最近1小時內的成交，代表「此時此刻的行情」。
      ...
      const current = avgInRange(0, 1);
      const oneDay = avgInRange(1, 48);
      ...
      pop.innerHTML = '<p class="craft-mat-worlds-title">均價比較</p>' +
        ...
        '<p class="craft-muted" style="font-size:10px;margin-top:4px">「當前」是最近1小時內的成交；「近1天」是1~48小時（當天＋昨天）；「近3/7/30天」是過去那整段時間所有成交的平均。</p>';
    } catch (e) {
