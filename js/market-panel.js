...
(function () {
  ...
  function openCraftSolverModal(rid) {
    ...
    craftSection.classList.add('market-modal-scene');
    if (!craftSection.querySelector('.market-modal-scene-close')) {
      const closeBtn = document.createElement('button');
      closeBtn.type = 'button';
      closeBtn.className = 'market-modal-scene-close';
      closeBtn.innerHTML = '<i class="ph ph-x"></i>';
      closeBtn.addEventListener('click', function () { craftSection.classList.remove('market-modal-scene'); });
  ...
  async function loadMarketSection(itemId) {
    ...
    // hq-only／世界篩選／手續費都是純畫面篩選或計算，不需要重新查價，切換時只重繪表格本身就好
    let hqOnly = false, includeFee = false, worldFilter = 'ALL';
    function renderListingTable() {
      let rows = hqOnly ? overview.listings.filter(function (l) { return l.hq; }) : overview.listings;
      if (worldFilter !== 'ALL') rows = rows.filter(function (l) { return l.world === worldFilter; });
      ...
      $('mk-listing-table-slot').innerHTML = rows.map(function (l) {
    ...
    $('mk-world-filter').addEventListener('change', function () { worldFilter = this.value; renderListingTable(); });
  }
  ...
  const HOT_METRICS = [
    ...
    { key: 'changePct', label: '漲跌幅度', fmt: function (v) { return (v >= 0 ? '+' : '') + v.toFixed(1) + '%'; }, hint: '短期動能：最近24小時的成交均價，比24~48小時前那段的均價貴/便宜多少' },
  ];
    ...
    body.innerHTML =
      ...
      '<div class="market-hot-list">' +
        top.map(function (r, i) {
          ...
          // 第9點：線性比例會被最大值那一筆徹底壓扁其他所有長條（例如8萬 vs 2xxx vs 3xx，
          // 線性算出來後面幾乎都貼著0%看不出差異）。改用開根號比例——這是處理長尾分佈資料的
          // 常見手法，把最大值的視覺優勢壓縮一點，其餘數值之間的相對差異才看得出來。
          const barPct = Math.max(4, Math.round((Math.sqrt(Math.abs(v)) / Math.sqrt(maxVal)) * 100));
