...
(function () {
  ...
  async function loadMarketSection(itemId) {
    ...
    if (badgeParts.length) html += '<div class="market-stat-bar">' + badgeParts.join('') + '<button type="button" class="market-history-btn market-stat-watch-btn" id="mk-watch-btn"></button></div>';
    else html += '<div class="market-stat-bar"><button type="button" class="market-history-btn market-stat-watch-btn" id="mk-watch-btn"></button></div>';
    ...
    html += '<p class="market-subheading">目前掛單（共 ' + overview.listings.length + ' 筆） ' +
      ...
      overview.listings.map(function (l) {
        return '<tr><td>' + l.pricePerUnit.toLocaleString() + '金</td><td>' + l.quantity.toLocaleString() + '</td><td>' + (l.hq ? '<span class="market-hq-tag">HQ</span>' : '<span class="market-nq-tag">NQ</span>') + '</td><td>' + l.world + '</td></tr>';
    ...
    if (!history || !history.length) {
      ...
      body.innerHTML = '<table class="market-price-table"><thead><tr><th>成交價</th><th>數量</th><th>品質</th><th>世界</th><th>時間</th></tr></thead><tbody>' +
        history.slice(0, 50).map(function (h) {
          return '<tr><td>' + h.pricePerUnit.toLocaleString() + '金</td><td>' + h.quantity.toLocaleString() + '</td><td>' + (h.hq ? '<span class="market-hq-tag">HQ</span>' : '<span class="market-nq-tag">NQ</span>') + '</td><td>' + h.world + '</td><td class="craft-muted">' + timeAgo(h.timestamp) + '</td></tr>';
        }).join('') + '</tbody></table>';
    }
  ...
  function renderSupplyChain(itemId, rid, box) {
    ...
    function facetV(a, b, bendY, tipFrom, phase) {
    ...
    matNodes.forEach(function (n, i) {
      ...
      facetV(a, b, bendY2, { x: a.x, y: bendY2 }, 'down');
    ...
    useNodes.forEach(function (n, i) {
      ...
      facetV(a, b, bendY1, { x: a.x, y: bendY1 }, 'up');
      svgParts += cardHtml(x, useTop, n.itemId, n.name, '', ' data-mk-goto-item="' + n.itemId + '"', false);
    });
    if (useOverflow) {
  ...
  function renderSupplyCrumb(box) {
    ...
    if (!el) return;
    if (detailBreadcrumb.length <= 1) { el.innerHTML = ''; return; } // 只有自己一層時，麵包屑點了也回到同一頁，沒有存在意義，直接不顯示
    const lastIdx = detailBreadcrumb.length - 1;
    el.innerHTML = detailBreadcrumb.map(function (id, i) {
      const nm = ITEM_NAMES_TW_ALL[id] || id;
      const sep = i > 0 ? '<span class="craft-muted"> › </span>' : '';
      // 最後一項是「目前正在看的物品」，點了也只是原地重繪同一頁，所以不做成連結，純文字顯示就好
      return sep + (i === lastIdx
        ? '<span class="market-crumb-current">' + nm + '</span>'
        : '<span class="market-crumb-item" data-mk-crumb-idx="' + i + '">' + nm + '</span>');
    }).join('');
