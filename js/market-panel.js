...
(function () {
  ...
  function renderTrendChart(pts) {
    if (!pts || pts.length < 2) return '<p class="craft-muted" style="padding:12px 0">這個區間內沒有足夠的成交紀錄可以畫圖，換個區間試試。</p>';
  ...
  const HOT_METRICS = [
    { key: 'velocity', label: '賣速', fmt: function (v) { return v.toFixed(1) + ' 件/天'; }, hint: '流動性：多快能賣掉，數字越高代表越搶手' },
    { key: 'changePct', label: '漲跌幅度', fmt: function (v) { return (v >= 0 ? '+' : '') + v.toFixed(1) + '%'; }, hint: '短期動能：最近成交比較早的成交貴/便宜多少（粗略估算，不是嚴謹統計）' },
    { key: 'totalValue', label: '總交易金額', fmt: function (v) { return Math.round(v).toLocaleString() + ' 金'; }, hint: '熱錢規模：賣得快但單價低的道具，跟真正熱錢在流動的道具，用這個指標才分得出差異' },
    { key: 'volatility', label: '波動度', fmt: function (v) { return v.toFixed(1) + '%'; }, hint: '投機空間：目前最高價比最低價貴多少，價差大代表低買高賣的空間大' },
  ];
  ...
  async function runHotScan(scope, metric) {
    ...
    body.innerHTML = '<p class="craft-muted">掃描中，先抓最近有市場活動的道具，再批次查詳細數據，可能要幾秒鐘⋯</p>';
    try {
      ...
      const stats = await MarketData.fetchMarketStatsBatch(candidateIds, scopeName);
      const rows = Object.keys(stats).map(function (id) {
        const st = stats[id];
        return { itemId: id, name: ITEM_NAMES_TW_ALL[id] || ('#' + id), stats: st };
      }).filter(function (r) { return r.stats.velocity > 0 || r.stats.totalValue > 0; });
