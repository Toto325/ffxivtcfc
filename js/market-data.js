...
window.MarketData = (function () {
  ...
  async function fetchHistoryBatch(itemIds, scopeName, mode) {
    ...
    const result = {};
    const CHUNK = 15; // 第8點：DC範圍偶爾還是會失敗，批次再縮小一點，降低單次請求的資料量
    for (let i = 0; i < itemIds.length; i += CHUNK) {
      const chunk = itemIds.slice(i, i + CHUNK);
      const url = API + '/' + encodeURIComponent(scopeName) + '/' + chunk.join(',') + '?listings=0&entries=30';
      let data;
      try {
        const res = await throttledFetch(url);
        data = await res.json();
      ...
      list.forEach(function (entry) {
        ...
        // 第9點：漲跌幅度改成比較「近3天成交均價」跟「近7天成交均價」（累積區間，跟均價彈窗
        // 3/7/30天那套邏輯一致），mode='1v3'時改成比較「近1天」跟「近3天」，讓使用者自己切換。
        ...
        const shortDays = mode === '1v3' ? 1 : 3;
        const longDays = mode === '1v3' ? 3 : 7;
        const shortAvg = avgWithinDays(shortDays), longAvg = avgWithinDays(longDays);
