...
window.MarketData = (function () {
  ...
  async function fetchFullHistory(itemId, maxDays) {
    await ensureMeta();
    ...
    for (let page = 0; page < MAX_PAGES; page++) {
      let url = API + '/history/' + encodeURIComponent(settings.dcName) + '/' + itemId + '?entriesToReturn=1800';
  ...
  /* ── 熱度排行用：批次查「近期成交紀錄」，只為了算漲跌幅度用，不再查掛單listings——
   * 第7點拿掉了總交易金額／波動度兩個指標，這兩個都要用到掛單資料，不需要了。
   * 第8點抓到的真正原因：這裡本來讀 entry.regularSaleVelocity 這個欄位當賣速，但這個欄位
   * 跟物品詳情頁真正在用的「聚合端點」(aggregated, fetchSaleVelocityBatch) 算出來的數字對不上，
   * 兩者明顯不是同一套邏輯/同一個時間窗口。現在賣速直接呼叫fetchSaleVelocityBatch，
   * 跟物品詳情頁共用同一份「已經驗證過是對的」邏輯，不再自己另外讀一個來源不明的欄位。 ── */
  async function fetchHistoryBatch(itemIds, scopeName, mode) {
    if (!itemIds.length) return {};
    const result = {};
    const CHUNK = 20;
    const chunks = [];
    for (let i = 0; i < itemIds.length; i += CHUNK) chunks.push(itemIds.slice(i, i + CHUNK));
    // 第6點抓到的真正原因：這裡原本是for迴圈裡await，一批一批「排隊等」，200個候選道具切成
    // 十幾批，每批都要等前一批完全回來才發下一批，加上失敗重試會讓某幾批多等一次，累加起來
    // 就是你看到的「掃描好幾分鐘」。改成用Promise.all同時發出去，交給throttledFetch自己控制
    // 最多同時3個連線在跑，不用自己在這裡硬性排隊等待，總時間大幅縮短。
    await Promise.all(chunks.map(async function (chunk) {
      const url = API + '/' + encodeURIComponent(scopeName) + '/' + chunk.join(',') + '?listings=0&entries=60';
      let data;
      try {
        const res = await throttledFetch(url);
        data = await res.json();
      ...
      list.forEach(function (entry) {
        ...
        const history = (entry.recentHistory || []).slice();
        // 第9點抓到的真正原因：這裡跟均價彈窗（點物品進去看的那個）用的是兩套不同的資料來源——
        // 均價彈窗用fetchFullHistory，會自動翻頁湊滿完整天數；這裡為了掃200個候選物品的效能，
        // 只用一般批次端點抓entries=60筆，成交熱絡的道具很可能撐不到完整7天份，算出來的
        // 「近7天均價」其實只是「entries=60筆能涵蓋到的範圍」，跟均價彈窗的真正7天不是同一件事，
        // 這就是兩邊數字對不上的原因，不是公式寫錯。這裡先把entries從30提高到60改善一些，
        // 但受限於「一次掃200個道具」的效能考量，沒辦法比照均價彈窗那樣做到完全準確的翻頁。
        const now = Date.now() / 1000;
        function avgWithinDays(days) {
          const list2 = history.filter(function (h) { return h.timestamp >= now - days * 86400; });
          return list2.length ? list2.reduce(function (s, h) { return s + h.pricePerUnit; }, 0) / list2.length : null;
        }
        // 第8點：「近1天」用跟均價彈窗一致的定義＝1~48小時（當天＋昨天），不是單純的最近24小時
        function avgOneDay() {
          const list2 = history.filter(function (h) { return h.timestamp >= now - 48 * 3600 && h.timestamp < now - 1 * 3600; });
          return list2.length ? list2.reduce(function (s, h) { return s + h.pricePerUnit; }, 0) / list2.length : null;
        }
        const shortAvg = mode === '1v3' ? avgOneDay() : avgWithinDays(3);
        const longAvg = mode === '1v3' ? avgWithinDays(3) : avgWithinDays(7);
        const changePct = (shortAvg != null && longAvg != null && longAvg > 0) ? ((shortAvg - longAvg) / longAvg) * 100 : null;
        result[entry.itemID] = { changePct: changePct, shortAvg: shortAvg, longAvg: longAvg, saleCount: history.length };
      });
    }));
    return result;
  }
  ...
  async function fetchSaleVelocityBatch(itemIds, regionOverride) {
    if (!itemIds.length) return {};
    await ensureMeta();
    if (!settings.dcName) throw new Error('尚未設定資料中心，請先在設定裡選擇伺服器');
