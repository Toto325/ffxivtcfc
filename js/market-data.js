...
window.MarketData = (function () {
  ...
  async function fetchSaleVelocityBatch(itemIds) {
    ...
    const result = {};
    itemIds.forEach(function (id) {
      const hit = velocityCache.get(id);
    ...
    // 聚合端點文件上限跟其他端點一樣是100個itemId，這裡沿用專案裡其他地方已經在用的保守值90
    const CHUNK = 90;
    for (let i = 0; i < need.length; i += CHUNK) {
      const chunk = need.slice(i, i + CHUNK);
      const url = API + '/aggregated/' + encodeURIComponent(settings.dcName) + '/' + chunk.join(',');
      let data;
      try {
        const res = await throttledFetch(url);
        data = await res.json();
      ...
      (data.results || []).forEach(function (r) {
        ...
        velocityCache.set(r.itemId, { time: now, data: entry });
      });
      (data.failedItems || []).forEach(function (id) {
        ...
        result[id] = empty;
        velocityCache.set(id, { time: now, data: empty });
      });
    }
    return result;
  }
