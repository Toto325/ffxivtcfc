...
window.MarketData = (function () {
  ...
  async function fetchHistoryBatch(itemIds, scopeName) {
    ...
    await Promise.all(chunks.map(async function (chunk) {
      ...
      list.forEach(function (entry) {
        ...
        // 第5點：漲跌一律用「當前」比「近1天」——當前＝1~24小時，近1天＝1~48小時，
        // 不再提供3比7天／1比3天的切換選項。
        const now = Date.now() / 1000;
        function avgInHours(fromH, toH) {
          const list2 = history.filter(function (h) { return h.timestamp >= now - toH * 3600 && h.timestamp < now - fromH * 3600; });
          return list2.length ? list2.reduce(function (s, h) { return s + h.pricePerUnit; }, 0) / list2.length : null;
        }
        const shortAvg = avgInHours(1, 24);
        const longAvg = avgInHours(1, 48);
