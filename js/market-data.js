...
window.MarketData = (function () {
  ...
  async function fetchHistoryBatch(itemIds, scopeName) {
    ...
    for (let i = 0; i < itemIds.length; i += CHUNK) {
      ...
      const url = API + '/' + encodeURIComponent(scopeName) + '/' + chunk.join(',') + '?listings=0&entries=20';
      let data;
      try {
        const res = await throttledFetch(url);
        data = await res.json();
      } catch (e) {
        // 第5點：偶爾查不到資料，很可能是暫時性的請求失敗（尤其資料中心範圍封包較大），
        // 先自動重試一次，不用每次都靠使用者手動按重新整理。
        try {
      ...
      list.forEach(function (entry) {
        ...
        // 第8點：漲跌幅度改成「當前均價」對比「1天前均價」——當前均價＝最近24小時內成交均價，
        // 1天前均價＝24~48小時前那個區間的成交均價，是實際的期間對期間比較。
        const now = Date.now() / 1000;
        const currentWindow = history.filter(function (h) { return h.timestamp >= now - 1 * 86400; });
        const priorWindow = history.filter(function (h) { return h.timestamp < now - 1 * 86400 && h.timestamp >= now - 2 * 86400; });
        function avgPrice(list) { return list.length ? list.reduce(function (s, h) { return s + h.pricePerUnit; }, 0) / list.length : null; }
        const currentAvg = avgPrice(currentWindow), priorAvg = avgPrice(priorWindow);
        const changePct = (currentAvg != null && priorAvg != null && priorAvg > 0) ? ((currentAvg - priorAvg) / priorAvg) * 100 : null;
        result[entry.itemID] = { changePct: changePct, currentAvg: currentAvg, priorAvg: priorAvg, saleCount: history.length };
      });
    }
    return result;
  }
