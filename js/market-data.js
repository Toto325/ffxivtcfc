...
window.MarketData = (function () {
  ...
  async function fetchListingsBatch(itemIds) {
    ...
    for (let i = 0; i < need.length; i += CHUNK) {
      ...
      const url = API + '/' + encodeURIComponent(settings.dcName) + '/' + chunk.join(',') + '?listings=50&entries=0&fields=itemId%2Clistings.worldName%2Clistings.pricePerUnit%2Clistings.quantity%2Clistings.hq';
      let data;
      try {
        const res = await fetch(url);
        data = await res.json();
      } catch (e) { continue; } // 這批查詢失敗就跳過，不讓整個功能因為一批失敗而全部掛掉
      const items = chunk.length === 1 ? { items: [data] } : data; // 單一itemId時Universalis回傳格式跟多個不同，這裡統一成同一種形狀處理
      const list = chunk.length === 1 ? [data] : (data.items || []);
