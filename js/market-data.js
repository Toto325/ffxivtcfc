...
window.MarketData = (function () {
  ...
  /* ── 熱度排行用：批次查詳細市場數據（賣速、價格區間、近期成交，用來算漲跌動能跟總交易金額）。
   * 跟 fetchListingsBatch 很像，但那支特意把 entries 設成0（不要成交紀錄，省流量），這裡就是
   * 需要成交紀錄才特地另外寫一支，兩支不合併是為了不要讓「只是要查最低價」的一般用途也被迫
   * 多下載一堆用不到的歷史資料。 ── */
  async function fetchMarketStatsBatch(itemIds, scopeName) {
    if (!itemIds.length) return {};
    const result = {};
    const CHUNK = 90;
    for (let i = 0; i < itemIds.length; i += CHUNK) {
      const chunk = itemIds.slice(i, i + CHUNK);
      const url = API + '/' + encodeURIComponent(scopeName) + '/' + chunk.join(',') + '?listings=20&entries=20';
      let data;
      try {
        const res = await throttledFetch(url);
        data = await res.json();
      ...
      list.forEach(function (entry) {
        ...
        const history = (entry.recentHistory || []).slice().sort(function (a, b) { return b.timestamp - a.timestamp; });
        const minPrice = listings.length ? listings[0].pricePerUnit : null;
        const maxPrice = listings.length ? listings[listings.length - 1].pricePerUnit : null;
        // 漲跌幅度：用「最近一半筆成交」跟「較早一半筆成交」的均價比較，抓短期動能，
        // 不是嚴謹的統計模型，只是一個「最近是不是在漲/跌」的粗略訊號。
        let changePct = null;
        if (history.length >= 4) {
          const half = Math.floor(history.length / 2);
          const recentAvg = history.slice(0, half).reduce(function (s, h) { return s + h.pricePerUnit; }, 0) / half;
          const olderAvg = history.slice(half).reduce(function (s, h) { return s + h.pricePerUnit; }, 0) / (history.length - half);
          if (olderAvg > 0) changePct = ((recentAvg - olderAvg) / olderAvg) * 100;
        }
        const totalValue = history.reduce(function (s, h) { return s + h.pricePerUnit * h.quantity; }, 0);
        result[entry.itemID] = {
          itemId: entry.itemID,
          velocity: entry.regularSaleVelocity || 0,
          minPrice: minPrice, maxPrice: maxPrice,
          avgPrice: entry.currentAveragePrice || null,
          changePct: changePct,
          totalValue: totalValue,
          volatility: (minPrice && maxPrice && (minPrice + maxPrice) > 0) ? ((maxPrice - minPrice) / ((minPrice + maxPrice) / 2)) * 100 : null,
        };
      });
    }
    return result;
  ...
  return {
    ...
    fetchMarketStatsBatch: fetchMarketStatsBatch,
