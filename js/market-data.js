  ...
  async function fetchHistoryBatch(itemIds, scopeName) {
    ...
    for (let i = 0; i < itemIds.length; i += CHUNK) {
      ...
      list.forEach(function (entry) {
        ...
        // 第9點：漲跌幅度改成「不同時間窗口的均價對比」，不是拿成交筆數對半分——
        // 拿「最近3天」的成交均價，對比「再往前3~7天」那個區間的成交均價，是實際的期間對期間比較，
        // 不是含糊的「前半vs後半筆數」（筆數對半分在成交不均勻時，兩邊代表的實際天數可能差很多，
        // 算出來的%數字很難對應到玩家自己在成交紀錄上肉眼看到的漲跌）。
        const now = Date.now() / 1000;
        const recentWindow = history.filter(function (h) { return h.timestamp >= now - 3 * 86400; });
        const priorWindow = history.filter(function (h) { return h.timestamp < now - 3 * 86400 && h.timestamp >= now - 7 * 86400; });
        function avgPrice(list) { return list.length ? list.reduce(function (s, h) { return s + h.pricePerUnit; }, 0) / list.length : null; }
        const recentAvg = avgPrice(recentWindow), priorAvg = avgPrice(priorWindow);
        const changePct = (recentAvg != null && priorAvg != null && priorAvg > 0) ? ((recentAvg - priorAvg) / priorAvg) * 100 : null;
        result[entry.itemID] = { changePct: changePct, recentAvg: recentAvg, priorAvg: priorAvg, saleCount: history.length };
      });
    }
    return result;
  }
