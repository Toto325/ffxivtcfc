...
(function () {
  ...
  /* ══ fetchFashion（改版：讀取自家 data/fashion.json，不再經由 CORS proxy）
   * data/fashion.json 由 GitHub Actions 排程執行 scripts/fetch_fashion.py 產生，
   * 格式為 { source: '原文網址', posts: [ {author,uid,floor,segments}, ... ] }
   * ════════════════════════════════════════════════════════════════════ */
  app.fetchFashion = async function () {
    ...
    var self = this;
    try {
      var r = await fetch('data/fashion.json', { cache: 'no-store' });
      if (!r.ok) throw new Error('status ' + r.status);
      var data  = await r.json();
      var final = (data && data.posts) || [];
      if (!final.length) {
      ...
      console.warn('[XIV] fetchFashion error:', e.message);
      ctn.innerHTML = '<div class="news-error"><span>暫時無法讀取，<button class="fashion-retry-btn" onclick="app._retryFashion()">重新讀取</button>　或　<a href="' + BASE + '" target="_blank" rel="noopener noreferrer" style="color:var(--gold)">前往原文查看</a></span></div>';
    }
  };
